# -*- coding: utf-8 -*-
"""Physik-Gesamtheft Klasse 7 Realschule: vier Kapitel, Test hinter jedem Kapitel.
...
DREI ENTSCHEIDUNGEN DES AUFTRAGGEBERS SIND HIER UMGESETZT
    ...
    3  Der Messwerte-Anhang BLEIBT im Schuelerband. Die Forscherseiten verweisen
       ...
       den Lehrerband wandern, sonst zeigt jede der 36 Seiten ins Leere.
"""
...
# Klasse 7 hat keine Schaltzeichen-Seite und keine eingebauten Schaltbilder.
SCHALTBILD={}
SZ_NACH=None

def _diag(chid,tid):
    # Klasse 7 arbeitet ausschliesslich mit eigenen Einstiegsbildern, nicht mit
    # gezeichneten Szenen. Die Rueckfallebene aus Klasse 5 entfaellt deshalb.
    return None

BANDNAME="FELO Physik 7 · Realschule NRW"
DATEINAME="FELO_Physik_7_Realschule_NRW"
FUSS="FELO Physik 7 · Forscherheft"
...
# der Satzspiegel des Blattes und traegt ueber 100 Zeichen je Zeile. Gemessen am
# Musterband 8: 29 % aller gefuellten Lesezeilen lagen darueber, die laengste bei
...
# dafuer, dass 36 gedruckte Codes weiter gelesen werden - bewusst so gelassen.
...
def _versalwoerter_im_fliesstext():
    ...
    # Nicht nur die Inhaltsdateien: Die Stichworte des Kernlehrplans werden
    # GEDRUCKT (Trennseite und "Basiskonzepte im Ueberblick"), und sie bringen
    # eigene Abkuerzungen mit. In Klasse 7 sind das IR und UV aus "Farbspektrum
    # (IR bis UV)" - die Versalprobe meldete sie im ersten Lauf 4x als Verstoss,
    # obwohl sie im Lehrplan genau so stehen und keine Raetselwoerter sind.
    for ch in CHAPTERS:
        _f=lehrplan.feld(ch["id"])
        if _f: flach([_f[1], _f[2]])
...
def einpassen(bauer,nmin,nmax):
    ...
    meisten Schreibzeilen. Grund fuer die erste (gemessen im Musterband 8): Mit
    acht Zeilen je Weiterdenken-Aufgabe rutschte eine einzige Linie auf ein
    zweites Blatt - das Folgeblatt trug nichts als die Fortsetzungszeile und
    einen Strich, viermal im Band. Mit sieben passt die Einheit auf ein Blatt.
...
def datenblatt(h,d,y,daten):
    ...
    Klasse 7 hat keine solche Seite - alle 36 Themen tragen eine Simulation.
    Der Renderer steht trotzdem im neuen Satz da: die uebrigen Baende der Reihe
    brauchen ihn, und er wird mit einer Probeseite gegengelesen."""
    n=len(daten["spalten"])
...
def ch_test(test,chtitel,pn):
    ...
    def bauer(nl):
        """Der Test mit nl Schreibzeilen in Aufgabe 4 und 5."""
        ...
        def b_kopfzeile(h,d,y):
            ...
            yy=y+12; xx=fd.X0+20
            for lab,br in (("Name:",340),("Klasse: 7",0),("Datum:",200)):
        ...
        return B
    # Der Test wird eingepasst wie die Test-Vorbereitung. Ohne das trug der Test
    # zu Kapitel 3 eine dritte Seite, auf der nur noch Notenspiegel und Schluss-
    # zeile standen - 18 % gefuellt, gemessen an book_p151 des ersten Laufs.
    # einpassen() nimmt zuerst die wenigste Seitenzahl und darin die meisten
    # Schreibzeilen: Kapitel 1, 2 und 4 bekommen dadurch MEHR Zeilen als vorher
    # (7 / 7 / 5 statt 5 / 4 / 4), Kapitel 3 dafuer 3 statt 5 - und spart die Seite.
    # Untergrenze 3 ist dieselbe, die hier schon vorher mit max(...,3) galt.
    B,_nl=einpassen(bauer,3,8)
...
def book_cover():
    ...
    y+=24
    fd.T(h,fd.X0,y,"Physik 7",fd.schrift("bold",DISPLAY_MITTEL),fd.STIL["h1"])
    y+=LH_DISPLAY+6
    ...
    fd.T(h,fd.X0,y,"Die vier Kapitel",fd.schrift("bold",fd.KASTEN_KOPF),fd.STIL["akzent"])
    h.ln([(fd.X0,y+LH_KOPF+2),(fd.X1,y+LH_KOPF+2)],fd.STIL["zart"],1.2)
    y+=LH_KOPF+22
...
def about_pages(pn):
    ...
    B.append(b_para("Dieses Heft nimmt Schülerinnen und Schüler als Forscherinnen und Forscher ernst. Statt fertige Antworten "
    ...
    pillars=[
     ...
     ("Nah am Alltag","Alltagssituationen und Schülersprache; die hervorgehobene Forscherfrage könnten die Schülerinnen und Schüler selbst gestellt haben."),
...
# ═════════════════════════════════════════════════════════════════════════════
# Spaltenanteile der Basiskonzept-Tabelle. GEMESSEN, nicht geschaetzt:
# "Wechselwirkung" ist ein Wort ohne Leerzeichen, es kann nicht umbrechen, und
# fd.tabelle verkleinert nichts - bei 0,22 stehen 150,6 Einheiten zur Verfuegung,
# das Wort braucht 171 und lief in allen vier Kapiteln ueber den Trennstrich in
# die Nachbarspalte. 0,26 gibt 183,8 Einheiten. Die breiteste Zelle der zweiten
# Spalte ist "Massenanziehung," mit 191,5 Einheiten und hat bei 0,50 noch 383.
# Die Zahl steht hier und nicht im Aufruf, damit die Breitenprobe genau diese
# misst und nicht eine Abschrift davon.
BK_ANTEILE=[0.26,0.50,0.24]
...
#  (in diesem Band 8 Seiten: je Kapitel eine Wortgitter- und eine Kreuzwort-
#  seite). Die Funktionen und assessment.json
...
def footer(h,pn,acc):
    ...
    h.T(ML+34,H-48,"PHYSIK · KLASSE 7 · FORSCHERHEFT",COP(9),GOLD_D)
...
def ch_kreuzwort(entries,name,pn,acc):
    ...
    # Die Tipps sind laenger als in Klasse 5. Passt die laengere der beiden Spalten
    # nicht mehr bis ueber die Fusszeile, wird der Satz stufenweise enger gestellt,
    # statt unten aus der Seite zu laufen.
    def _hoehe(gr,lh,ab):
...
def _loes_forscherseiten(ch,ci,pn0):
    ...
    for j,idx in enumerate(teile):
        ...
        for k,i in enumerate(idx):
            ...
            #
            # Gesammelt wird der BEREICH, nicht der volle Code. Vorher stand hier
            # die Liste der Codes ("UF1", "E5", "K7", "B2") und darunter wurde
            # gefragt, ob "UF" / "E" / "K" / "B" darin vorkommt - das trifft nie
            # zu, und deshalb druckte KEINE Loesungsseite auch nur ein einziges
            # Kaertchen, obwohl das Deckblatt "Kompetenzbereiche je Kapitel"
            # verspricht. Gefunden am gesetzten Blatt, nicht im Protokoll.
            def _bereich(c):
                return "UF" if c.startswith("UF") else c[0]
            _ber=[]
            for _b in ([ (o.get("aufgabe") or {}).get("komp"), o.get("alltagKomp") ]
                       + list((UEBD.get(tid,{}) or {}).get("komp") or [])):
                if not _b: continue
                _r=_bereich(_b)
                if _r not in _ber: _ber.append(_r)
            _xr=W-ML
            # Von rechts nach links gezeichnet - also die Reihenfolge umdrehen,
            # damit gelesen UF · E · K · B dasteht, die Reihenfolge des Lehrplans.
            for _b in ["B","K","E","UF"]:
...
def lb_cover():
    ...
    h.tracked(W/2,330,"PHYSIK 7",DIDOT(46),INK,4)
...
def build_lehrerband():
    ...
    try:
        ...
        w.add_metadata({"/Title":"FELO PHYSIK 7 – Realschule NRW – Lehrerband",
                        "/Author":"Abdullah Lala","/Creator":"FELO",
                        "/Subject":"Lösungen und Erwartungshorizonte zum Forscherheft Klasse 7"})
...
def build_ebook(src, dst, nav, qrpages=None, seitentexte=None,
    ...
    w._root_object[NameObject("/PageMode")]=NameObject("/UseOutlines")
    w.add_metadata({"/Title":"FELO PHYSIK 7 – Forscherheft Realschule NRW",
                    "/Author":"Abdullah Lala",
                    "/Subject":"Optik · Spiegel, Brechung & Farben · Sonne, Planeten und Schwerkraft · Sterne, Galaxien und der Anfang",
                    "/Keywords":"FELO, Physik, Klasse 7, Realschule NRW, Forscherheft, Optik, Weltall, Fassung 1.1, Stand 2026-08-28",
                    "/Creator":"FELO"})
...
if __name__=="__main__":
    ...
    # sichtbar werden sie nur so. Im Musterband 8 trug eine Seite einmal genau
    # eine einzige Schreiblinie.
