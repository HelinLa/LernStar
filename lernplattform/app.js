...
// ---- Schulform ----
const SCHOOL_TYPES = {
  grundschule:    { id:'grundschule',    label:'Grundschule',     grades:[1,2,3,4],         color:'#EC4899' },
  weiterfuehrend: { id:'weiterfuehrend', label:'Weiterf. Schule', grades:[5,6,7,8,9,10,11,12,13], color:'#7C3AED' },
};
let activeSchoolType = localStorage.getItem('ls_school_type') || 'weiterfuehrend';
...
const state = {
  ...
  modulStep: 1,
  modulVorwissenIdx: 0,
  modulVorwissenAnswers: [],
  modulDiagnose: { idx:0, score:0, answers:[] },
...
const GRADE_GRADIENTS = {
  klasse1:  'linear-gradient(135deg,#EC4899,#F9A8D4)',
  klasse2:  'linear-gradient(135deg,#F59E0B,#FCD34D)',
  klasse3:  'linear-gradient(135deg,#10B981,#6EE7B7)',
  klasse4:  'linear-gradient(135deg,#3B82F6,#93C5FD)',
...
function renderHome() {
  ...
  // Schulform-Tabs
  const tabsContainer = document.getElementById(‘schoolTypeTabs’);
  if (tabsContainer) {
    tabsContainer.innerHTML = Object.values(SCHOOL_TYPES).map(st => `
      <button class="sf-tab ${activeSchoolType === st.id ? ‘active’ : ‘’}"
              style="--sf-color:${st.color}"
              onclick="setSchoolType(‘${st.id}’)">
        ${st.id === ‘grundschule’ ? ‘🌱’ : ‘🎓’} ${st.label}
      </button>`).join(‘’);
  }

  // Klassen der aktiven Schulform filtern
  const allowedNums = SCHOOL_TYPES[activeSchoolType]?.grades || [];
  const grid = document.getElementById(‘gradeGrid’);
  grid.innerHTML = ‘’;
  Object.values(CONTENT)
    .filter(grade => allowedNums.includes(grade.num))
    .forEach(grade => {
      const subjectNames = grade.subjects.slice(0,2).map(s => s.name);
      const progress = getGradeProgress(grade.id);
      const isGs = grade.num <= 4;
      const card = document.createElement(‘div’);
      card.className = ‘grade-card’;
      card.style.background = GRADE_GRADIENTS[grade.id] || ‘linear-gradient(135deg,#6B7280,#9CA3AF)’;
      ...
      card.innerHTML = `
        <div class="grade-card-inner">
          ...
          ${isGs ? ‘<div class="gs-badge">🌟 Modulgenerator</div>’ : ‘’}
          ${progress > 0 ? `<div style="margin-top:6px;font-size:.75rem;opacity:.8">✅ ${progress}% geschafft</div>` : ‘’}
...
function renderSubject() {
  ...
  subject.topics.forEach((t, i) => {
    if (t.isChapter) {
      ...
      ch.className = 'topic-chapter';
      ch.textContent = t.name;
      topicsList.appendChild(ch);
      return;
    }
    const tIdx = nonChapterIdx++;
    ...
    item.innerHTML = `
      ...
      ${t.kompetenz ? `<button class=”modul-start-btn” onclick=”startModul('${state.gradeId}','${state.subjectId}',${tIdx})”>&#127775; Lernmodul</button>` : ''}
      ${hasVideo && !t.kompetenz ? `<button class=”topic-play-btn” id=”topicBtn${i}” onclick=”playTopic(${i})”>&#128266; Erklaeren</button>` : ''}
      ${EV_SCENES[t.name] ? `<button class=”ev-topic-btn” onclick=”openErklaerVideo('${t.name.replace(/'/g,”\\'”)}')”>&#127909; Erklaervideo</button>` : ''}`;
...
function startModul(gradeId, subjectId, topicIdx) {
  ...
  if (!subject) return;
  const topic = subject.topics.filter(t => !t.isChapter)[topicIdx];
  if (!topic || !topic.kompetenz) return;
  const ex = subject.exercises.find(e => e.ersteAnwendung) || subject.exercises[0] || null;
  ...
  state.modulDiagnose  = { idx:0, score:0, answers:[] };
  state.modulKompetenz = { idx:0, score:0, answers:[] };
  state.modulErsteAnw  = { idx:0 };
  state.modulReflexion = null;
  state.modulExerciseId = ex?.id || null;
  navigate('modul', gradeId, subjectId);
}

function _modulGetTopic() {
  const grade   = CONTENT[state.modulGradeId];
  const subject = grade?.subjects.find(s => s.id === state.modulSubjectId);
  if (!subject) return null;
  return subject.topics.filter(t => !t.isChapter)[state.modulTopicIdx];
}
...
function _modulGetExercise() {
  ...
  const subject = grade?.subjects.find(s => s.id === state.modulSubjectId);
  if (!subject) return null;
  if (state.modulExerciseId) return subject.exercises.find(e => e.id === state.modulExerciseId);
  return subject.exercises.find(e => e.ersteAnwendung) || subject.exercises[0] || null;
}

const MODUL_STEP_LABELS = [
  '', 'Kompetenzziel', 'Vorwissen aktivieren', 'Erklaerung', 'Erklaerungsvideo',
  'Erste Anwendung', 'Mini-Diagnose', 'Individuelle Uebungen',
  'Transferaufgabe', 'Reflexion', 'Kompetenznachweis'
];
...
function renderModul() {
  ...
  const grade   = CONTENT[state.modulGradeId];
  const subject = grade?.subjects.find(s => s.id === state.modulSubjectId);
  const step = state.modulStep;
  const pct  = Math.round(((step - 1) / 10) * 100);
  const container = document.getElementById('modulContainer');
  let html = '<div class="modul-header" style="background:' + (GRADE_GRADIENTS[state.modulGradeId] || '#7C3AED') + '">'
    + '<div class="hero-breadcrumb" onclick="navigate(\'subject\',\'' + state.modulGradeId + '\',\'' + state.modulSubjectId + '\')">'
    + '&larr; ' + (grade?.label || '') + ' / ' + (subject?.name || '') + '</div>'
    + '<h2 class="modul-title">' + topic.name + '</h2>'
    + '<div class="modul-progress-bar"><div class="modul-progress-fill" style="width:' + pct + '%"></div></div>'
    + '<div class="modul-step-indicator">Schritt ' + step + ' / 10 &mdash; ' + MODUL_STEP_LABELS[step] + '</div>'
    + '</div><div class="modul-body">';
  if (step === 1)  html += _modulStep1(topic);
  else if (step === 2)  html += _modulStep2(topic);
  else if (step === 3)  html += _modulStep3(topic);
  else if (step === 4)  html += _modulStep4(topic);
  else if (step === 5)  html += _modulStep5();
  else if (step === 6)  html += _modulStep6();
  else if (step === 7)  html += _modulStep7();
  else if (step === 8)  html += _modulStep8(topic);
  else if (step === 9)  html += _modulStep9();
  else if (step === 10) html += _modulStep10();
  html += '</div>';
  container.innerHTML = html;
}

function _modulStep1(t) {
  return '<div class="modul-step-card"><div class="modul-icon">&#127919;</div><h3>Das kann ich am Ende:</h3>'
    + '<div class="modul-kompetenz">' + t.kompetenz + '</div>'
    + '<p class="modul-hint">Lies das Ziel genau durch. Was kannst du schon?</p>'
    + '<button class="btn-modul-next" onclick="modulNext()">Weiter &rarr;</button></div>';
}
...
function _modulStep2(t) {
  ...
  if (idx >= q.length)
    return '<div class="modul-step-card"><div class="modul-icon">&#10003;</div><h3>Vorwissen aktiviert!</h3>'
      + '<button class="btn-modul-next" onclick="modulNext()">Zur Erklaerung &rarr;</button></div>';
  return '<div class="modul-step-card"><div class="modul-icon">&#129504;</div>'
    + '<h3>Frage ' + (idx+1) + ' von ' + q.length + '</h3>'
    + '<div class="modul-vorfrage">' + q[idx] + '</div>'
    + '<p class="modul-hint">Ueberlege kurz &ndash; es gibt keine falsche Antwort!</p>'
    + '<button class="btn-modul-next" onclick="modulVorwissenWeiter()">Ich habe nachgedacht &rarr;</button></div>';
}

function modulVorwissenWeiter() { state.modulVorwissenIdx++; renderModul(); }

function _modulStep3(t) {
  const bsp = (t.beispiele || []).map(b => '<li>' + b + '</li>').join('');
  const err = (t.typischeFehler || []).map(f => '<li>&#9888;&#65039; ' + f + '</li>').join('');
  return '<div class="modul-step-card modul-erklaerung"><h3>&#128214; Erklaerung</h3>'
    + '<div class="modul-text">' + t.explanation + '</div>'
    + (bsp ? '<div class="modul-box modul-beispiele"><strong>&#10003; Beispiele:</strong><ul>' + bsp + '</ul></div>' : '')
    + (err ? '<div class="modul-box modul-fehler"><strong>Typische Fehler:</strong><ul>' + err + '</ul></div>' : '')
    + (t.merksatz ? '<div class="modul-merksatz">&#128161; <strong>Merke:</strong> ' + t.merksatz + '</div>' : '')
    + '<button class="btn-modul-next" onclick="modulNext()">Verstanden &rarr;</button></div>';
}

function _modulStep4(t) {
  if (!t.videoskript) return '<div class="modul-step-card"><p>Kein Videoskript.</p>'
    + '<button class="btn-modul-next" onclick="modulNext()">Weiter &rarr;</button></div>';
  const szenen = t.videoskript.split(/Szene \d+\s*[–\-]/).filter(s => s.trim());
  const sHtml = szenen.length
    ? szenen.map((s, i) => '<div class="ev-szene"><div class="ev-szene-num">&#127909; Szene ' + (i+1) + '</div>'
        + '<div class="ev-szene-text">' + s.trim() + '</div></div>').join('')
    : '<p>' + t.videoskript + '</p>';
  return '<div class="modul-step-card"><h3>&#127909; Erklaerungsvideo-Skript</h3>'
    + '<div class="ev-skript-container">' + sHtml + '</div>'
    + '<button class="btn-modul-next" onclick="modulNext()">Weiter &rarr;</button></div>';
}

function _modulStep5() {
  const ex = _modulGetExercise();
  const aufg = ex?.ersteAnwendung;
  if (!aufg || aufg.length === 0) return '<div class="modul-step-card"><p>Keine Aufgaben.</p>'
    + '<button class="btn-modul-next" onclick="modulNext()">Weiter &rarr;</button></div>';
  const idx = state.modulErsteAnw.idx;
  if (idx >= aufg.length) return '<div class="modul-step-card"><div class="modul-icon">&#10003;</div>'
    + '<h3>Erste Anwendung abgeschlossen!</h3>'
    + '<button class="btn-modul-next" onclick="modulNext()">Zur Diagnose &rarr;</button></div>';
  const a = aufg[idx];
  return '<div class="modul-step-card"><div class="modul-icon">&#9999;&#65039;</div>'
    + '<h3>Aufgabe ' + (idx+1) + ' von ' + aufg.length + '</h3>'
    + '<div class="modul-aufgabe">' + a.aufgabe + '</div>'
    + '<div class="modul-hint">&#128161; Hinweis: ' + a.hinweis + '</div>'
    + '<div id="ersteAnwLoesung" class="modul-loesung hidden">&#10003; Loesung: <strong>' + a.loesung + '</strong></div>'
    + '<div class="modul-btn-row">'
    + '<button class="btn-modul-hint" onclick="document.getElementById(\'ersteAnwLoesung\').classList.remove(\'hidden\')">Loesung zeigen</button>'
    + '<button class="btn-modul-next" onclick="modulErsteAnwWeiter()">Naechste &rarr;</button>'
    + '</div></div>';
}
...
function _modulStep6() {
  const ex = _modulGetExercise();
  const diag = ex?.minidiagnose;
  if (!diag || diag.length === 0) return '<div class="modul-step-card"><p>Keine Diagnose.</p>'
    + '<button class="btn-modul-next" onclick="modulNext()">Weiter &rarr;</button></div>';
  const { idx, score } = state.modulDiagnose;
  if (idx >= diag.length) {
    const pct = Math.round((score / diag.length) * 100);
    const col = pct >= 80 ? '#059669' : pct >= 50 ? '#D97706' : '#DC2626';
    const lbl = pct >= 80 ? '&#10003; Sicher' : pct >= 50 ? 'Teilweise sicher' : 'Noch unsicher';
    return '<div class="modul-step-card"><h3>&#128269; Diagnose-Ergebnis</h3>'
      + '<div class="modul-diagnose-result" style="color:' + col + '">' + lbl + '<br><span style="font-size:2rem;font-weight:900">' + pct + '%</span></div>'
      + '<button class="btn-modul-next" onclick="modulNext()">Weiter &rarr;</button></div>';
  }
  const f = diag[idx];
  const optsHtml = f.options.map((o, i) =>
    '<button class="modul-option-btn" onclick="modulDiagnoseAnswer(' + i + ')">' + String.fromCharCode(65+i) + ') ' + o + '</button>'
  ).join('');
  return '<div class="modul-step-card"><h3>&#128269; Frage ' + (idx+1) + ' / ' + diag.length + '</h3>'
    + '<div class="modul-aufgabe">' + f.q + '</div>'
    + '<div class="modul-options">' + optsHtml + '</div></div>';
}

function modulDiagnoseAnswer(chosen) {
  const ex = _modulGetExercise();
  const f = ex.minidiagnose[state.modulDiagnose.idx];
  if (chosen === f.correct) state.modulDiagnose.score++;
...
function _modulStep7() {
  const { score, answers } = state.modulDiagnose;
  const ex = _modulGetExercise();
  const total = ex?.minidiagnose?.length || 1;
  const pct = Math.round((score / total) * 100);
  if (pct >= 80) return '<div class="modul-step-card"><div class="modul-icon">&#127775;</div>'
    + '<h3>Individuelle Uebungen</h3><p>Du hast ' + pct + '% in der Diagnose &ndash; sehr gut! Direkt weiter.</p>'
    + '<button class="btn-modul-next" onclick="modulNext()">Zur Transferaufgabe &rarr;</button></div>';
  const falsch = answers.filter(a => !a.correct);
  return '<div class="modul-step-card"><div class="modul-icon">&#128170;</div>'
    + '<h3>Individuelle Uebungen</h3><p>' + falsch.length + ' Frage(n) nochmal anschauen:</p>'
    + '<ul class="modul-retry-list">' + falsch.map(a => '<li>&#10060; ' + a.q + '</li>').join('') + '</ul>'
    + '<div class="modul-merksatz">&#128161; Schau dir Schritt 3 nochmal an oder frage Herr Lala im Chat.</div>'
    + '<button class="btn-modul-hint" onclick="state.modulStep=3;renderModul()">&#8592; Zur Erklaerung</button>'
    + '<button class="btn-modul-next" onclick="modulNext()">Weiter &rarr;</button></div>';
}

function _modulStep8(t) {
  const tr = t.transfer;
  if (!tr) return '<div class="modul-step-card"><p>Keine Transferaufgabe.</p>'
    + '<button class="btn-modul-next" onclick="modulNext()">Weiter &rarr;</button></div>';
  return '<div class="modul-step-card"><div class="modul-icon">&#127758;</div>'
    + '<h3>Transferaufgabe: ' + tr.kontext + '</h3>'
    + '<div class="modul-aufgabe">' + tr.aufgabe + '</div>'
    + '<div class="modul-hint">&#128161; Tipp: ' + tr.tipp + '</div>'
    + '<p>Bearbeite die Aufgabe im Heft. Klicke auf Weiter, wenn du fertig bist.</p>'
    + '<button class="btn-modul-next" onclick="modulNext()">Fertig! &rarr;</button></div>';
}

function _modulStep9() {
  if (state.modulReflexion) {
    const map = { sicher:'&#10003; Sicher', teilweise:'Teilweise sicher', unsicher:'Noch unsicher' };
    return '<div class="modul-step-card"><h3>&#129300; Deine Einschaetzung</h3>'
      + '<p>Du hast angegeben: <strong>' + map[state.modulReflexion] + '</strong></p>'
      + '<button class="btn-modul-next" onclick="modulNext()">Zum Kompetenznachweis &rarr;</button></div>';
  }
  return '<div class="modul-step-card"><div class="modul-icon">&#129300;</div>'
    + '<h3>Reflexion: Wie sicher fuehlst du dich?</h3>'
    + '<div class="modul-reflexion-buttons">'
    + '<button class="modul-reflex-btn reflex-sicher" onclick="modulSetReflexion(\'sicher\')">&#10003; Sicher</button>'
    + '<button class="modul-reflex-btn reflex-teilweise" onclick="modulSetReflexion(\'teilweise\')">&#9888; Teilweise</button>'
    + '<button class="modul-reflex-btn reflex-unsicher" onclick="modulSetReflexion(\'unsicher\')">&#10006; Noch unsicher</button>'
    + '</div></div>';
}

function modulSetReflexion(v) { state.modulReflexion = v; renderModul(); }

function _modulStep10() {
  const ex = _modulGetExercise();
  const fragen = ex?.kompetenznachweis;
  if (!fragen || fragen.length === 0) return '<div class="modul-step-card"><div class="modul-icon">&#127942;</div>'
    + '<h3>Kompetenznachweis</h3><p>Kein Abschlusstest &ndash; Modul abgeschlossen!</p>'
    + '<button class="btn-modul-next" onclick="modulAbschliessen()">Abschliessen &rarr;</button></div>';
  const { idx, score } = state.modulKompetenz;
  if (idx >= fragen.length) {
    const pct = Math.round((score / fragen.length) * 100);
    const passed = pct >= 60;
    if (passed) {
      const key = state.modulGradeId + '_' + state.modulSubjectId + '_modul_' + state.modulTopicIdx;
      state.progress[key] = pct;
      localStorage.setItem('ls_progress', JSON.stringify(state.progress));
    }
    return '<div class="modul-step-card"><div class="modul-icon">' + (passed ? '&#127942;' : '&#128218;') + '</div>'
      + '<h3>' + (passed ? 'Kompetenz erreicht!' : 'Noch etwas ueben...') + '</h3>'
      + '<div class="modul-diagnose-result" style="color:' + (passed ? '#059669' : '#DC2626') + '">' + pct + '%</div>'
      + '<div class="modul-btn-row">'
      ...
      + '<button class="btn-modul-next" onclick="modulAbschliessen()">Zum Fach &rarr;</button>'
      + '</div></div>';
  }
  const f = fragen[idx];
  const optsHtml = f.options.map((o, i) =>
    '<button class="modul-option-btn" onclick="modulKompetenzAnswer(' + i + ')">' + String.fromCharCode(65+i) + ') ' + o + '</button>'
  ).join('');
  return '<div class="modul-step-card"><h3>&#127942; Frage ' + (idx+1) + ' / ' + fragen.length + '</h3>'
    + '<div class="modul-aufgabe">' + f.q + '</div>'
    + '<div class="modul-options">' + optsHtml + '</div></div>';
}

function modulKompetenzAnswer(chosen) {
  const ex = _modulGetExercise();
  const f = ex.kompetenznachweis[state.modulKompetenz.idx];
