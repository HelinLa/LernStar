...
Klasse, Schulform und Quellband kommen aus plan.py. Diese Fassung ist die der
Baende 7, 8 und 9 plus den DATENBLATTSEITEN, die es nur in Band 10 gibt - alles
Zusaetzliche haengt an `cfg["daten"]` und schlaeft in den anderen Baenden. Sie
koennte die drei anderen also ersetzen; getan wird das erst, wenn jemand die
drei Baende danach wirklich nachgemessen hat.
...
DAS DATENBLATT WIRD NICHT MEHR VERKLEINERT.
    Vier Einheiten dieses Bandes haben keine Simulation, sondern ein gedrucktes
    Datenblatt (plan.DATENBLATT: fv11, fn11, fn13, fn15). Der alte Renderer
    zwaengte es in eine 440 Einheiten schmale Randspalte und suchte sich dazu
    einen Schriftgrad, der passte - 16,5 Einheiten sind 7,9 pt, das Sicherheits-
    netz ging bis 12,5 Einheiten = 6,0 pt. Beides liegt unter der 10-pt-Grenze
    der Groessentafel. Jetzt steht das Blatt auf der LESESPALTE und im
    Foerdergrad (14 pt); zu lange Zellen brechen UM, wie in jeder anderen
    Tabelle des Moduls. Das kostet Hoehe - dafuer laesst es sich lesen.
"""
import os, sys, json
...
# ZEICHEN, keine von Einheiten: 830 Einheiten tragen bei 12 pt rund 81 Zeichen,
# bei 10 pt aber 97 - gemessen an den gesetzten Blaettern lagen genau dort die
# einzigen drei Zeilen ueber 85 Zeichen (Impressum und Deckblattzeile). Die
# Rechnung geht ueber SPALTE_65, die im Modul fuer 65 Zeichen bei FLIESS steht.
...
def qr_pfad(tid):
    """Eigener fv/fn-Code hat Vorrang; solange er fehlt, springt der Code der
    Quellseite ein - er loest live dieselbe Simulation auf.

    Eine Datenblattseite bekommt KEINEN Code. Der Rueckgriff auf die Quellseite
    wuerde ihr sonst den Code des Regelhefts unterschieben, und der zeigt auf
    eine Simulation, von der auf dieser Seite kein Wort steht."""
    if SEITEN.get(tid,{}).get("daten"): return None
...
def b_zeilen(text,art="reg",grad=None,breite=None,punkt=False,
    ...
    Der Erwartungshorizont von fn12 laeuft ueber 62 Zeilen, also ueber mehr als
    eine ganze Seite. Ein Baustein, der nirgends hinpasst, wird vom Umbruch auf
    ein frisches Blatt gelegt und laeuft dort unten heraus - gemessen endeten so
    im ersten Lauf 13 Lehrerseiten auf 1754 (Blattkante) statt auf 1674, und 20
    weitere trugen 7 % Text, weil sie nur die Ueberschrift davor hielten.
    Genau dieser Fehler stand schon einmal im Buch (CLAUDE.md, "Der Lehrerteil
    lief unter der Blattkante weiter") - der alte Renderer brach deshalb
    zeilenweise um. Diese Fassung tut dasselbe, nur mit den Mitteln des Moduls:
...
# ═════════════════════════════════════════════════════════════════════════════
#  DATENBLATT  (nur Band 10: Einheiten ohne Simulation)
# ═════════════════════════════════════════════════════════════════════════════
# Anteile der drei Spalten. Uebernommen aus dem alten Renderer, weil die
# Inhalte danach geschrieben sind: links die Sache, in der Mitte die laengste
# Aussage, rechts die kurze Angabe.
DB_ANTEILE=[0.36,0.34,0.30]


def datenblatt(h,d,y,st,dat):
    """Das gedruckte Datenblatt einer Seite ohne Simulation.

    Es steht auf der LESESPALTE und im Foerdergrad, nicht mehr in einer schmalen
    Randspalte auf 7,9 pt. Zellen brechen UM (tabelle() weiter oben), sie werden
    nicht verkleinert - das alte Sicherheitsnetz zog eine einzelne zu breite
    Zeile bis auf 6,0 pt herunter, ohne dass es jemandem auffiel.

    Unter dem Blatt stehen zwei Zeilen, beide aus dem Inhalt:
      quelle - woher die Zahlen stammen, in KLEIN
      merke  - die Lesehilfe. Sie nimmt das Ergebnis NIE vorweg, sonst nimmt sie
               Schritt 2 die Aufgabe weg (FOERDER_PROFIL.md, "Datenblattseiten")."""
    st=st or fd.STIL
    y=tabelle(h,d,y,st,dat["spalten"],dat["zeilen"],DB_ANTEILE)
    if dat.get("quelle"):
        y=fd.para(h,fd.KASTEN_X0,y+10,dat["quelle"],fd.schrift("reg",fd.KLEIN),
                  st["text"],LESE,fd.LH_KLEIN)
    if dat.get("merke"):
        y=fd.para(h,fd.KASTEN_X0,y+8,dat["merke"],fd.schrift("med",FOE),
                  st["akzent"],LESE,LH_FOE)
    return y
...
def bausteine_a(cfg,luft=0.0):
    ...
    # ③ Forschen am Bildschirm  -  auf Datenblattseiten: Auswerten & beurteilen
    dat=cfg.get("daten")
    B.append(b_marke(3,"Auswerten & beurteilen" if dat else "Forschen am Bildschirm"))
    if dat:
        # Erst das Blatt, dann die Schritte: die Schritte sagen "Lies im
        # Datenblatt ...", und wer das liest, soll es schon vor sich haben.
        # Der Kasten traegt dieselbe Bauart wie der Bildschirmkasten
        # (Doppelrahmen), aber ein GEZEICHNETES Gitter statt des Dreiecks -
        # "▤" (U+25A4) kennt SourceSans3 nicht und druckte ein leeres Kaestchen.
        B.append(bst("Datenblattmarke",
                     lambda h,d,y: kasten_bildschirm(h,d,y,fd.STIL,
                         ["Datenblatt: "+dat.get("titel",""),""],
                         zeichner=gitterzeichen),
                     fd.ABS_AUFGABE,haftet=1))
        B.append(bst("Datenblatt",
                     lambda h,d,y: datenblatt(h,d,y,fd.STIL,dat),
                     fd.ABS_AUFGABE))
    else:
...
def seite_l(cfg,pn):
    ...
    dieses Bandes ist hoeher als eine ganze Seite - fn12 traegt einen
    Erwartungshorizont ueber 62 Zeilen. Wer nur zwischen Punkten trennt, legt ihn
    auf ein frisches Blatt und laesst ihn dort unten wieder herauslaufen."""
