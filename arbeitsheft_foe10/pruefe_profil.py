...
# Einheiten, die der Bauplan ohne Simulation fuehrt: Dort MUSS ein Datenblatt
# stehen, und nur dort darf eines stehen.
DATENBLATT_IDS = set(getattr(_plan, "DATENBLATT", ()))
...
# ── Datenblattbreite: mit der echten Schrift messen, nicht Zeichen zaehlen ──
# Beim ersten Anlauf habe ich die Spaltenbreite in Zeichen GESCHAETZT und lag um
# den Faktor 2 daneben: build_final laedt jede Schrift mit S=2 doppelt so gross
# und teilt die gemessene Breite wieder durch S. Zeichenzahlen sind hier also
# wertlos - gemessen wird mit derselben Schrift, die auch druckt.
# Mehrwortzellen brechen um, das ist erlaubt. Gefaehrlich ist nur ein EINZELNES
# Wort, das breiter ist als seine Spalte: h.wrap() kann ein Wort nicht trennen,
# es laesst die Zeile stehen und sie laeuft in die Nachbarspalte.
#
# STAND 09.09.2026 - der Band steht im neuen Satz (felo_design):
#   vorher   440 Einheiten breit, 16,5 Einheiten Schrift (7,9 pt), Randspalte;
#            ein zu breites Wort wurde still bis auf 6,0 pt verkleinert.
#   jetzt    830 Einheiten (die Lesespalte), 29,17 Einheiten Schrift (14 pt);
#            verkleinert wird gar nichts mehr, Zellen brechen um.
# Die neuen Spalten sind relativ WEITER als die alten (Breite x1,99 gegen
# Schrift x1,76) - was vorher passte, passt also weiter. Gemessen tragen die
# drei Spalten einzeilig 266,8 / 250,2 / 217,0 Einheiten.
DB_SCHRIFT_PX = 58        # einheiten(14) = 29,17, mal S=2, gerundet
DB_BREITE     = 830.0     # Lesespalte KASTEN_X1-KASTEN_X0
DB_ANTEILE    = (0.36, 0.34, 0.30)
DB_INNEN      = 32        # Luft je Zelle in tabelle() (h.wrap(..., breite*a-32))
...
def _spaltenmass():
    ...
    fdir = os.path.join(os.path.dirname(HERE), "arbeitsheft", "fonts")
    try:
        # Genau die Schrift und der Grad, mit denen build_pilot.tabelle() setzt.
        reg = ImageFont.truetype(os.path.join(fdir, "SourceSans3-Regular.ttf"), DB_SCHRIFT_PX)
        fet = ImageFont.truetype(os.path.join(fdir, "SourceSans3-Bold.ttf"), DB_SCHRIFT_PX)
    except OSError:
        return None
    d = ImageDraw.Draw(Image.new("RGB", (8, 8)))
    innen = [DB_BREITE * a - DB_INNEN for a in DB_ANTEILE]
    return lambda t, sp, fett=False: (d.textlength(t, font=fet if fett else reg) / 2.0,
                                      innen[sp])

BREITE_MESSEN = _spaltenmass()
...
# Ausgenommen sind nur Schluessel der OBERSTEN Ebene: Verwaltungsangaben, die
# nicht gedruckt werden (seite.quelle = "rk12", seite.sim, seite.theme ...) und
# der Bildauftrag. Frueher galt die Liste auf JEDER Ebene - damit fiel auch
# seite.daten.quelle heraus, und die wird gedruckt (build_pilot.datenblatt setzt
# sie unter das Blatt). Die vier Datenblattseiten wiegen deshalb je fuenf bis
# zwanzig Woerter mehr als vorher - nachgemessen mit wortgewicht() am
# 05.09.2026: fn11 380 (alte Regel 375), fn13 378 (373), fn15 397 (377),
# fv11 379 (372). Ein frueherer Stand dieses Kommentars nannte hier 388 und 383;
# beide Zahlen waren falsch. Wer die Regel wieder anfasst, misst nach, statt die
# Zahlen fortzuschreiben - im naechsten Budgetstreit wird mit ihnen argumentiert.
_OHNE = ("bildauftrag", "id", "sim", "quelle", "theme", "sicherheit")

def wortgewicht(seite):
    n = 0
    def go(x, key=None, oben=False):
        nonlocal n
        if isinstance(x, str):
            if not (oben and key in _OHNE): n += len(x.split())
        elif isinstance(x, dict):
            for k, v in x.items(): go(v, k, oben=(x is seite))
        elif isinstance(x, list):
            for v in x: go(v, key, oben)
    go(seite)
    return n
...
OPERATOREN = ("Wähle", "Stelle", "Lies", "Trage", "Drücke", "Vergleiche",
              ...
              # Datenblattseiten: der Dreischritt heisst ablesen - ordnen - beurteilen
              "Beurteile", "Entscheide", "Begründe")
...
def pruefe(einheit):
    ...
    # ── Datenblattseiten (Band 10: Seiten ohne Simulation) ──────────
    dat = s.get("daten")
    # Ohne diese Abfrage rutscht eine Datenblatteinheit, der das Feld ganz fehlt,
    # geraeuschlos durch: Abschnitt 3 waere dann leer und die Seite hielte einen
    # QR-Code bereit, den es nicht gibt.
    if s.get("id") in DATENBLATT_IDS and dat is None:
        f.append("Diese Einheit ist im Bauplan eine Datenblattseite, hat aber kein Feld daten")
    if dat is not None and s.get("id") not in DATENBLATT_IDS:
        f.append("Feld daten, aber der Bauplan sieht hier eine Simulation vor")
    if dat is not None:
        if s.get("sim"):
            f.append("Datenblattseite darf kein Feld sim tragen (sonst wird ein QR-Code gesetzt)")
        if len(dat.get("spalten", [])) != 3:
            f.append(f"Datenblatt braucht GENAU 3 Spalten (hat {len(dat.get('spalten', []))})")
        dz = dat.get("zeilen", [])
        if not (2 <= len(dz) <= 5):
            f.append(f"Datenblatt hat {len(dz)} Zeilen (erlaubt 2–5)")
        for i, r in enumerate(dz):
            if len(r) != 3:
                f.append(f"Datenblatt-Zeile {i+1} hat {len(r)} Zellen statt 3")
        # Laengere Zellen brechen um - das ist erlaubt, aber ab 60 Zeichen wird
        # die Zelle drei Zeilen hoch und die Tabelle sprengt die Seite.
        for i, r in enumerate(dz):
            for j, z in enumerate(r):
                if len(str(z)) > 60:
                    f.append(f"Datenblatt-Zelle {i+1}/{j+1} hat {len(str(z))} Zeichen (höchstens 60)")
        # Einzelne Woerter gegen die echte Spaltenbreite messen (Kopfzeile fett).
        if BREITE_MESSEN:
            for zi, r in enumerate([dat.get("spalten", [])] + list(dz)):
                for j, z in enumerate(r[:3]):
                    for wort in str(z).split():
                        br, platz = BREITE_MESSEN(wort, j, fett=(zi == 0))
                        if br > platz:
                            wo = "Kopfzeile" if zi == 0 else f"Zeile {zi}"
                            f.append(f"Datenblatt {wo}, Spalte {j+1}: „{wort}“ ist "
                                     f"{br:.0f} breit, die Spalte nur {platz:.0f} – "
                                     "das Wort läuft über die Spalte hinaus, bitte kürzen")
        if not dat.get("merke"):
            f.append("Datenblatt braucht eine Merke-Zeile (Lesehilfe, nie das Ergebnis)")
        if s.get("ersatz"):
            f.append("Datenblattseite braucht keinen Simulations-Ersatzsatz")
    ...
    # ── Druckbarkeit ────────────────────────────────────────────────
    # Geprueft wird die GANZE Einheit, nicht nur die Schuelerseite: Der
    # Lehrerband wird mit derselben SourceSans3 gesetzt, und seine Texte zitieren
    # die Simulation woertlich. Im Faktendump zu fv3 endet eine Rueckmeldung auf
    # "🙂" (U+1F642) - waere dieses Zitat in den Lehrerteil gewandert, haette es
    # stumm ein leeres Kaestchen gedruckt, genau wie damals "▤" im Renderer.
    if CMAP:
        for t in _texte(einheit):
            for c in t:
...
def selbsttest(gut):
    ...
    if CMAP:
        ...
        # Zweite Probe für dieselbe Regel, aber im LEHRERTEIL: Der Band setzt ihn
        # mit derselben Schrift, und die Zeichenschleife sah ihn früher nicht an.
        kaputt(("lehrer", "a1", "weg"), "Rückmeldung: Mit der Uhrzeit hat es nichts zu tun 🙂",
               "fehlt in der Schrift")

    # Kaputt-Proben Datenblatt (Band 10). Sie brauchen eine Kennung aus dem
    # Bauplan, deshalb wird die gute Einheit einmal umbenannt.
    if DATENBLATT_IDS:
        db_id = sorted(DATENBLATT_IDS)[0]
        proben[0] += 1
        k = copy.deepcopy(gut); k["seite"]["id"] = k["lehrer"]["id"] = db_id
        _, bef = pruefe(k)
        if not any("kein Feld daten" in b for b in bef):
            fehler.append(f"Kaputt-Probe Datenblatt fehlt ({db_id}) → keine Meldung (Befunde: {bef})")

        proben[0] += 1
        k = copy.deepcopy(gut)
        k["seite"]["daten"] = {"titel": "T", "spalten": ["a", "b", "c"],
                               "zeilen": [["1", "2", "3"], ["4", "5", "6"]], "merke": "m"}
        _, bef = pruefe(k)
        if not any("Bauplan sieht hier eine Simulation vor" in b for b in bef):
            fehler.append(f"Kaputt-Probe Datenblatt zu viel → keine Meldung (Befunde: {bef})")

        if BREITE_MESSEN:
            # Ein einzelnes zu breites Wort - h.wrap() kann es nicht trennen, die
            # Zeile bleibt stehen und laeuft in die Nachbarspalte. Gemessen ist
            # "Pumpspeicherkraftwerk" 293 Einheiten breit, Spalte 1 nur 267.
            proben[0] += 1
            k = copy.deepcopy(gut); k["seite"]["id"] = k["lehrer"]["id"] = db_id
            k["seite"]["daten"] = {"titel": "T", "spalten": ["Art", "Stoff", "Regelbar"],
                                   "zeilen": [["Pumpspeicherkraftwerk", "b", "c"],
                                              ["d", "e", "f"]], "merke": "m"}
            _, bef = pruefe(k)
            if not any("läuft über die Spalte hinaus" in b for b in bef):
                fehler.append(f"Kaputt-Probe zu breites Wort → keine Meldung (Befunde: {bef})")

    if CMAP:
        # Der Renderer-Zeichenprüfer bekommt eigene Proben: eine echte Setzzeile
        # mit fehlendem Zeichen muss auffallen, ein Trennstrich aus einem print()
        # und eine abgeschaltete Zeile dürfen es NICHT.
        proben[0] += 1
        r = renderer_zeichen([("probe.py", 1, '    h.T(x,y,"▤",AVB(9),CREAM)\n')])
        if not r:
            fehler.append("Renderer-Probe: fehlendes Zeichen in h.T(…) blieb unbemerkt")
        # Dieselbe Probe für die Schreibweise des neuen Satzes. Ohne sie könnte
        # jemand fd.T aus _SETZEN entfernen, ohne dass eine Probe rot wird.
        proben[0] += 1
        r = renderer_zeichen([("probe.py", 1,
                               '    fd.T(h,x,y,"▤",fd.schrift("bold",fd.KLEIN),fd.GRUND)\n')])
        if not r:
            fehler.append("Renderer-Probe: fehlendes Zeichen in fd.T(…) blieb unbemerkt")
        proben[0] += 1
        r = renderer_zeichen([("probe.py", 1,
                               '    fd.para(h,x,y,"▤",f,c,LESE,LH)\n')])
        if not r:
            fehler.append("Renderer-Probe: fehlendes Zeichen in fd.para(…) blieb unbemerkt")
        proben[0] += 1
        r = renderer_zeichen([("probe.py", 2, '    print("── Bericht ──")\n'),
                              ("probe.py", 3, '    h.T(x,y,"✎",AVM(13)) if False else None\n')])
        if r:
            fehler.append(f"Renderer-Probe: Fehlalarm auf print/toter Zeile ({r})")
    return fehler, proben[0]


# ── Undruckbare Zeichen IM RENDERER ──────────────────────────────────
# Die Zeichenprüfung oben sieht nur die Inhalte. Ein Zeichen, das im Setzskript
# selbst steht, rutscht durch: "▤" (U+25A4) als Abzeichen des Datenblatt-Kastens
# druckte ein leeres Kästchen, obwohl jede Einheit sauber war. Gefunden habe ich
# es erst an der gerenderten Seite - also wird auch der Renderer geprüft.
#
# Geprüft wird NUR, was wirklich auf die Seite gesetzt wird: Zeilen mit einem
# Aufruf von h.T / h.para / h.tracked / h.wrap. Beim ersten Anlauf prüfte ich
# jede Zeile und bekam fünf Fehlalarme aus fünf Meldungen – ein Trennstrich aus
# einem print() auf dem Terminal und eine tote Zeile mit „if False“.
#
# SEIT DEM NEUEN SATZ (09.09.2026) setzt der Renderer über das Gestaltungsmodul:
# fd.T() und fd.para() statt h.T() und h.para(). Ohne die beiden neuen Muster
# hätte dieser Prüfer ab sofort nur noch die paar verbliebenen h.-Aufrufe
# angesehen und weiter grün gemeldet – genau die Sorte Prüfer, die schweigt.
# Beide Formen sind im Selbsttest mit einer eigenen Probe belegt.
_SETZEN = ("h.T(", "h.para(", "h.tracked(", "h.wrap(", "fd.T(", "fd.para(")

def renderer_zeichen(zeilen=None):
    if not CMAP: return []
    quellen = zeilen
    if quellen is None:
        quellen = []
        for name in ("build_pilot.py", "build_book.py"):
            p = os.path.join(HERE, name)
            if not os.path.exists(p): continue
            quellen += [(name, nr, z) for nr, z in
                        enumerate(open(p, encoding="utf-8"), 1)]
    f = []
    for name, nr, zeile in quellen:
        if zeile.lstrip().startswith("#"): continue          # Kommentare drucken nicht
        if "if False" in zeile: continue                     # abgeschaltete Zeile
        if not any(s in zeile for s in _SETZEN): continue
        for c in zeile:
            if ord(c) > 0x7f and ord(c) not in CMAP:
                f.append(f"{name}:{nr} – Zeichen „{c}“ (U+{ord(c):04X}) "
                         "fehlt in der Schrift und druckt als leeres Kästchen")
    return f
