...
seiten, lehrer, fehlt, abweichend = [], [], [], []
for th in foeplan.THEMEN:
    ...
    # Gedruckt wird der Name aus der EINHEIT (build_pilot.kopf setzt cfg["name"]).
    # Eine Abweichung vom Bauplan faellt sonst still durch und steht danach im
    # gesetzten Heft, waehrend plan.py etwas anderes sagt (gefunden am 05.09.2026
    # an fn7: "der Fund" gegen "ein Fund"). Kein Abbruch - welche der beiden
    # Fassungen gilt, entscheidet der Inhalt der Seite, nicht dieses Skript.
    if d["seite"].get("name") and d["seite"]["name"] != th["name"]:
        abweichend.append(f"{th['id']}: Seite {d['seite']['name']!r} · plan.py {th['name']!r}")
...
for a in abweichend:
    print("HINWEIS: Seitenname weicht vom Bauplan ab —", a)
