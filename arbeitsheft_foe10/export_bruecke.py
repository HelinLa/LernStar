...
for ch in CHAPTERS:
    for tid in ch["topics"]:
        # Datenblattseiten haben keine Simulation und tragen deshalb auch keinen
        # gedruckten QR-Code. Ein Brueckeneintrag ohne sim koennte nur zur
        # Startseite fuehren - genau die tote Verknuepfung, die uns schon zweimal
        # eingeholt hat. Sie gehoeren also nicht in die Bruecke.
        if not SIM.get(tid): continue
        o = SEITEN[tid]
        eintraege.append({
            "id":        tid,
            "klasse":    plan.KLASSE,
