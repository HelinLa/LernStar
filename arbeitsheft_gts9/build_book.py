# -*- coding: utf-8 -*-
"""FELO PHYSIK · Gesamtschule NRW: Kapitel mit Test dahinter.

Klasse und Schulform kommen aus plan.py - diese Datei ist fuer ALLE VIER
Gesamtschulhefte (gts7, gts8, gts9, gts10) dieselbe und liegt in allen vier
Ordnern gleich. Wer sie aendert, kopiert sie danach in die drei anderen.
...
G/E-DIFFERENZIERUNG AB JAHRGANG 9 (APO-SI)
    Ein Thema mit "kurs": "E" traegt unter seiner Ueberschrift ein Abzeichen.
    Es heisst "E-Kurs" und nicht "E-KURS": die Vorgabe des neuen Satzes verbietet
    Woerter in Grossbuchstaben, und die Versalprobe am Ende dieses Skripts misst
    genau das an dem, was wirklich gedruckt wird.
"""
...
_spec=_ilu.spec_from_file_location("lehrplan_gts", os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "arbeitsheft", "lehrplan_gts.py"))
...
# ALLE Gesamtschulhefte gleich und muss beim naechsten Band nicht angefasst werden.
KL       = str(plan.KLASSE)                          # "7", "8", "9", "10"
SFORM    = plan.SCHULFORM                            # "Gesamtschule NRW"
SFORM_GR = SFORM.upper()                             # nur fuer den Lehrerband
SFORM_DT = SFORM.replace(" ", "_")                   # "Gesamtschule_NRW"
BANDNAME = f"FELO Physik {KL} · {SFORM}"
DATEINAME = f"FELO_Physik_{KL}_{SFORM_DT}"
...
# Inhalt kommt aus je EINER Quelldatei (Titel sind gebacken)
...
# Die Gesamtschulreihe hat keine Schaltzeichen-Seite und keine eingebauten Schaltbilder.
SCHALTBILD={}
SZ_NACH=None

def _diag(chid,tid):
    # Die Gesamtschulreihe arbeitet ausschliesslich mit eigenen Einstiegsbildern,
...
# Kapitel 1 von Band 8 lagen 29 % aller gefuellten Lesezeilen darueber, die
# laengste bei 107 Zeichen - genau der Gewinn, den die groessere Schrift bringen
# sollte. Schreiblinien, Kopf-, Fuss- und Trennlinien bleiben auf fd.X0..fd.X1:
# sie werden nicht gelesen, sondern beschrieben.
...
# dafuer, dass die gedruckten Codes weiter gelesen werden - bewusst so gelassen.
...
# Wie viele Kapitel traegt der Band? Das Deckblatt sagt es in Worten - und die
# Zahl steht in plan.py, nicht hier. Sonst behauptet das Deckblatt von Band 10
# "die vier Kapitel", waehrend zwei gesetzt sind.
_KAPITELWORT={1:"Das eine Kapitel",2:"Die zwei Kapitel",3:"Die drei Kapitel",
...
# (dort, wo niemand ein Raetsel setzt).
def _versalwoerter_im_fliesstext():
    ...
    # Der Kernlehrplan ist die ZWEITE gedruckte Textquelle des Bandes: seine
    # Stichworte stehen woertlich auf der Trennseite und in der Basiskonzept-
    # uebersicht. Ohne ihn meldete die Versalprobe "IR" aus "Spektrum des Lichts
    # (IR bis UV)" als Wort in Grossbuchstaben - waehrend das daneben stehende
    # "UV" durchging, nur weil es zufaellig auch im Heftinhalt vorkommt.
    for k in CHAPTERS:
        _f=lehrplan.feld(k["id"])
        if _f: roh.append(_f[1]); flach(_f[2])
...
# Die Klammerung der Bausteine (verklammern) steht im Modul: fd.umbrechen()
# rechnet sie seit dem 09.09.2026 selbst durch, damit alle Baende der Reihe sie
# haben und nicht jeder sie neu erfindet.
...
def einpassen(bauer,nmin,nmax):
    ...
    Aufgabe rutscht eine einzige Linie auf ein zweites Blatt - dieses traegt dann
    nichts als die Fortsetzungszeile und einen Strich.
...
def b_titel(text,breite=None,unterzeile=None,haftet=1,regel_bis=None,abzeichen=None):
    ...
    `abzeichen` traegt die G/E-Kennzeichnung ab Jahrgang 9. Es steht LINKS unter
    dem Titel und nicht rechts oben: dort sitzt auf Seite 1 der QR-Code."""
    ...
    def f(h,d,y):
        ...
        if abzeichen:
            fa=fd.schrift("bold",fd.KLEIN); ah=30
            bw=h.tw(abzeichen,fa)+28
            h.R(fd.X0,yy+2,fd.X0+bw,yy+2+ah,7,fill=fd.STIL["akzent"])
            fd.T(h,fd.X0+bw/2,yy+2+ah/2,abzeichen,fa,fd.STIL["akzent_schrift"],anchor="mm")
            yy+=ah+8
        if unterzeile:
...
def gitterzeichen(d,cx,cy,r,fill):
    ...
    Kaestchen (frueher build_book.py am Datenblatt-Kasten). Gezeichnet kann das
    nicht passieren."""
...
def datenblatt(h,d,y,daten):
    ...
    In dieser Reihe traegt nur Band 10 solche Seiten (fuenf Stueck). Die uebrigen
    Baende brauchen den Renderer trotzdem - es ist EINE Datei fuer vier Baende.
    Die Zellen werden UMGEBROCHEN, nicht verkleinert: fd.tabelle richtet die
    Zeilenhoehe nach der hoechsten Zelle."""
    n=len(daten["spalten"])
...
def topic_pages(cfg,chtitel,fno,pn):
    ...
    # Ab Jahrgang 9 traegt eine E-Kurs-Seite ihr Abzeichen unter dem Titel.
    B.append(b_titel(cfg.get("titel") or cfg["name"],
                     ...
                     abzeichen="E-Kurs" if cfg.get("kurs")=="E" else None))
...
def ch_test(test,chtitel,pn):
    ...
    # Die Tabelle haelt den Zuspruch fest. Ohne das rutschte in Band 10 allein
    # "Viel Erfolg - du schaffst das!" auf ein drittes Blatt: gemessen 233 von
    # 1674 Einheiten, eine zu 93 % leere Seite. Ueber die durchgerechnete
    # Klammerung nimmt der Unterkopf "Notenspiegel" beide mit.
...
def divider(num,ch,pn):
    ...
    del boxen[:]
    seiten=fd.umbrechen(B,hoehen)
    # Muss die Trennseite umbrechen, wird die Themenliste GLEICHMAESSIG verteilt.
    # Ein Gesamtschulkapitel traegt 14 bis 15 Themen; greedy setzte in Kapitel 2
    # von Band 7 zehn Kaestchen auf Seite 1 und vier auf Seite 2 - ein Blatt, das
    # zu drei Vierteln leer blieb (gemessen: 24 % gefuellt). Der erzwungene
    # Umbruch wird nur uebernommen, wenn er die Seitenzahl NICHT erhoeht.
    if len(seiten)>1 and len(themen_idx)>1:
        _t=set(themen_idx)
        auf1=sum(1 for i,_y in seiten[0] if i in _t)
        ziel=max(1,(len(themen_idx)+1)//2)
        if 0<ziel<auf1:
            _i=themen_idx[ziel-1]; _alt=B[_i].abstand
            B[_i].abstand=fd.UNTEN-fd.OBEN+1          # groesser als jede Seite
            neu=fd.umbrechen(B,hoehen)
            if len(neu)==len(seiten): seiten=neu
            else: B[_i].abstand=_alt
    raus=[]
    for si,eintraege in enumerate(seiten):
        im,d=newp(fd.GRUND); h=hp(im,d)
        marke=len(boxen)
...
def book_cover():
    ...
    fd.T(h,fd.X0,y,_KAPITELWORT.get(len(CHAPTERS),"Die Kapitel dieses Bandes"),
...
def about_pages(pn):
    ...
    B.append(b_para("Der Code steht rechts in der Kopfzeile jeder Aufgabe. Wortlaut nach: Kernlehrplan "
        "Naturwissenschaften, Gesamtschule NRW (Heft 3108).",art="med",breite=LESE,
    ...
    B.append(b_para("Er steht neben dem Kompetenzcode. Die drei Stufen folgen den Bildungsstandards; "
        "der Kernlehrplan Naturwissenschaften der Gesamtschule kennt sie nicht.",art="med",
...
def basiskonzept_seiten(pn):
    ...
    for ch in CHAPTERS:
        ...
        # 150,6 Einheiten - das Wort lief ueber die Trennlinie in die Nachbarspalte
        # (dieselbe Stelle steht auch im Musterband so). Gemessen, nicht geschaetzt:
        # laengstes Wort Spalte 1 = 171,0, Spalte 2 = 254,5 Einheiten.
...
def messwerte_pages(start_pn):
    ...
    # EINE durchlaufende Einheit, nicht eine je Kapitel. Kapitelweise gesetzt trug
    # die letzte Seite eines Kapitels oft einen einzigen Eintrag - ein Anhang darf
...
#  (in diesem Band zwei Seiten je Kapitel). Die Funktionen und assessment.json
...
def ch_kreuzwort(entries,name,pn,acc):
    ...
    # Auch die HOEHE begrenzen: Bei langen Fachwoertern wird das Gitter sonst so hoch,
    # dass die Tipps darunter auf Kleinstschrift zusammenschrumpfen.
...
# ═════════════════════════════════════════════════════════════════════════════
# Der Merksatz steht im Heft als Lueckensatz: pre, Luecke, post - und h.flow setzt
# zwischen JEDEN Baustein ein Leerzeichen. Im Lehrerband wird die Luecke gefuellt,
# also muss dieses Leerzeichen hier von Hand entstehen. Ohne es klebt die Loesung
# am naechsten Wort: "ergeben zusammen immer 100 Prozentdes Lichts." Faengt post
# dagegen mit einem Satzzeichen an (107 von 221 Faellen ein Punkt), darf KEIN
# Leerzeichen dazwischen. Gemessen an den vier Baenden: 161-mal Satzzeichen,
# 60-mal ein Wort.
_KEIN_ABSTAND = ".,;:!?)»“"
...
def _merksatz_ganz(m):
    p = m.get("post") or ""
    return (m["pre"]+" "+m["loesung"]+("" if p[:1] in _KEIN_ABSTAND else " ")+p).strip()
...
# Der Lehrerteil ist schon einmal unter der Blattkante weitergelaufen und hat
# still Pflichtbloecke verschluckt (arbeitsheft_foe9, 05.09.2026). Deshalb wird
# auch hier gemessen, am Bild und vor der Fusszeile.
LB_PROBE=[]
LB_UNTEN=H-96


def _lb_unterkante(im):
    bb=ImageChops.difference(im,Image.new("RGB",im.size,CREAM)).getbbox()
    return 0.0 if not bb else bb[3]/float(S)
...
def _loes_forscherseiten(ch,ci,pn0):
    ...
    for j,idx in enumerate(teile):
        ...
        for k,i in enumerate(idx):
            ...
            # Ab Jahrgang 9: welche Seiten sind E-Kurs-Stoff? Die Lehrkraft muss das
            # im Loesungsteil sehen, ohne im Schuelerband nachzuschlagen.
            if o.get("kurs")=="E":
                h.T(ML+46,y+28,"E-Kurs",AVB(15),ch["acc"])
            ...
            y+=luft
        LB_PROBE.append((f"Loesungen Forscherseiten Kapitel {ci+1} {j+1}/{len(teile)}",
                         pn0+j,_lb_unterkante(im)))
...
def _loes_uebungsseiten(ch,ci,pn0):
    """Loesungen zu Uebung, Test und Wortschatz - zweispaltig, ueber mehrere Seiten."""
    im,d=newp(CREAM); h=hp(im,d)                     # nur zum Ausmessen
    ...
    if a.get("prep",{}).get("mini"):
        ...
        # Loesung. Fuer Eltern, die zu Hause abfragen, standen die Fragen ohne Antwort.
        ...
        for j,x0 in ((0,Lx),(1,Rx)):
            ...
            for b in spalten[si+j]:
                if b[0]=="sec":
                    ...
                    y=yy+ab
        LB_PROBE.append((f"Loesungen Uebung/Test Kapitel {ci+1} {si//2+1}/{(len(spalten)+1)//2}",
                         pn0+len(seiten),_lb_unterkante(im)))
...
def lb_cover():
    ...
    im,d=newp(CREAM); h=hp(im,d)
    n_themen=sum(len(c["topics"]) for c in CHAPTERS)
    ...
    h.tracked(W/2,330,f"PHYSIK {KL}",DIDOT(46),INK,4)
    h.tracked(W/2,410,SFORM_GR,COP(16),GOLD_D,6); h.orn(W/2,452,150)
    z=[f"Lösungen zu allen {n_themen} Forscherseiten",
    ...
    if getattr(plan,"KURSE",False):
        z.append("E-Kurs-Seiten sind im Lösungsteil gekennzeichnet")
    yy=540
    for t in z:
...
def build_lehrerband():
    """Der Loesungsteil als eigenes PDF - Vorbild: arbeitsheft8/build_book.py."""
    del LB_PROBE[:]
    loes,_pn,starts=loesungen_pages(2)
    pages=[lb_cover()]+loes
    # ── Satzspiegel-Probe des Lehrerbandes ──
    _u=[p for p in LB_PROBE if p[2]>LB_UNTEN+0.5]
    print(f"Lehrerband · Satzspiegel-Probe: {len(LB_PROBE)} Loesungsseiten gemessen, "
          f"Grenze {LB_UNTEN:.0f} Einheiten")
    if _u:
        print(f"   WARNUNG: {len(_u)} Seite(n) laufen unter die Blattkante:")
        for name,seite,y in sorted(_u,key=lambda p:-p[2])[:10]:
            print(f"      Seite {seite:3d}  {name:46s} endet {y:.0f}  (+{y-LB_UNTEN:.0f})")
    elif LB_PROBE:
        _t=max(LB_PROBE,key=lambda p:p[2])
        print(f"   keine Seite laeuft unter die Blattkante. Tiefste: Seite {_t[1]} "
              f"({_t[0]}) endet {_t[2]:.0f}, {LB_UNTEN-_t[2]:.0f} Einheiten Luft.")
    # ── Zeichenprobe des Lehrerbandes ──
    # Er steht im alten, kompakten Satz und laeuft deshalb nicht durch fd.T. Sein
    # Fliesstext ist aber dieselbe Schrift; ein fehlendes Zeichen druckt hier
    # genauso ein leeres Kaestchen wie im Schuelerband.
    _lb_fehlt={}
    for _p in pages:
        for _x,_y,_b,_gr,_s in SEITENTEXTE.get(id(_p),[]):
            for _c in fd.fehlende_zeichen(_s,"reg"): _lb_fehlt[_c]=_lb_fehlt.get(_c,0)+1
    print("Lehrerband · Zeichenprobe: "+("0 fehlende Zeichen." if not _lb_fehlt else
          "FEHLENDE ZEICHEN: "+" ".join("U+%04X %s (%dx)"%(ord(c),c,n)
                                        for c,n in sorted(_lb_fehlt.items()))))
...
if __name__=="__main__":
    ...
    # Fast leere Seiten. Sie laufen unter keine Kante und es faellt kein Text weg -
    # sichtbar werden sie nur so.
