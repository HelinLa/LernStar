...
SCHREIBTISCH = ["Arbeitsheft-Bilder 10 Klasse", "Bilder für Arbeitsheft 10 Klasse",
                "Bilder fuer Arbeitsheft 10 Klasse"]
...
def _kennung(dateiname):
    ...
    # Kennung am Dateianfang - die Kennungen kommen aus DIESEM Heft, nicht aus einer
    # festen Liste. Vorher stand hier ein Muster nur fuer Klasse 5/6, damit hiess
    # "bg4 Tacho und Heft.png" fuer die uebrigen Hefte nichts.
    # Laengste zuerst, sonst schluckt "lt1" das "lt11".
    if _TAB is None: _TAB = _themen()
    for tid in sorted(set(_TAB.values()), key=len, reverse=True):
