# -*- coding: utf-8 -*-
"""Physik-Gesamtheft Klasse 10 Realschule: vier Kapitel, Test hinter jedem Kapitel.
...
DREI ENTSCHEIDUNGEN DES AUFTRAGGEBERS SIND HIER UMGESETZT
    ...
    3  Der Messwerte-Anhang BLEIBT im Schuelerband. Die Forscherseiten verweisen
       ...
       den Lehrerband wandern, sonst zeigt jede der 39 Seiten ins Leere.

BESONDERHEIT DIESES BANDES: ACHT DATENBLATTSEITEN
    ge6, ge11, ak11, ke1, ke2, ke3, ke8 und ke11 haben keine Simulation und
    tragen im Inhalt ein Feld "daten". Bei ihnen heisst Abschnitt 3 "Auswerten &
    beurteilen", der Quellenkasten nennt das Datenblatt statt der Simulation,
    und die Zahlen stehen als gedruckte Tabelle auf der Seite selbst. Diese acht
    Seiten tragen KEINEN QR-Code - dort gibt es nichts zu oeffnen.
"""
...
# Klasse 10 hat keine Schaltzeichen-Seite und keine eingebauten Schaltbilder.
SCHALTBILD={}
SZ_NACH=None

def _diag(chid,tid):
    # Klasse 10 arbeitet ausschliesslich mit eigenen Einstiegsbildern, nicht mit
    # gezeichneten Szenen. Die Rueckfallebene aus Klasse 5 entfaellt deshalb.
    return None

BANDNAME="FeLabs Physik 10 · Realschule NRW"
DATEINAME="FeLabs_Physik_10_Realschule_NRW"
FUSS="FeLabs Physik 10 · Forscherheft"
...
# dafuer, dass 39 gedruckte Codes weiter gelesen werden - bewusst so gelassen.
...
# (dort, wo niemand ein Raetsel setzt). Fuer Klasse 10 findet die Probe neun:
# CT, EU, INES, LED, PET und NS (das Formelzeichen der Sekundaerwindungszahl)
# sind echte Abkuerzungen; UF, II und III stehen nur deshalb dabei, weil die
# Kompetenz- und Anforderungscodes im Inhalt als Zeichenketten liegen.
...
def ausgleichen(B,hoehen):
    ...
    Platz zurueck. Fuer eine Kapitel-Trennseite ist es falsch: Die Kapitel
    dieses Bandes tragen 11, 11, 14 und 11 Themen, also mehr als ein Blatt fasst.
    Gemessen im ersten Lauf endete Seite 1 bei 1636 bis 1668 von 1674 Einheiten
    und auf Seite 2 stand nichts als die Leitzeile: 226 Einheiten, ein zu 94 %
    leeres Blatt, viermal im Band. (Klasse 8 kennt das nicht - dort hat kein
    Kapitel mehr als sieben Themen und jede Trennseite passt auf ein Blatt.)
...
def datenblatt(h,d,y,daten):
    ...
    ACHT Seiten dieses Bandes haben keine Simulation und tragen stattdessen
    gedruckte Zahlen: ge6, ge11, ak11, ke1, ke2, ke3, ke8, ke11. Die Tabelle
    steht wie jede andere auf der Lesespalte (fd.KASTEN_X0..fd.KASTEN_X1) -
    ueber die volle Blattbreite truege sie gut 100 Zeichen je Zeile.

    Zellen brechen UM, sie werden nicht verkleinert: das erledigt fd.tabelle.
    Gemessen an allen acht Datenblaettern passt kein einzelnes Wort ueber seine
    Spalte hinaus, das Sicherheitsnetz des alten Satzes (Schrift kleiner ziehen)
    wird also nicht gebraucht - und faellt damit weg."""
    n=len(daten["spalten"])
...
def ch_test(test,chtitel,pn,fuss=None):
    ...
    def b_kopfzeile(h,d,y):
        ...
        yy=y+12; xx=fd.X0+20
        for lab,br in (("Name:",340),("Klasse: 10",0),("Datum:",200)):
...
def book_cover():
    ...
    y+=24
    fd.T(h,fd.X0,y,"Physik 10",fd.schrift("bold",DISPLAY_MITTEL),fd.STIL["h1"])
    y+=LH_DISPLAY+6
...
def footer(h,pn,acc):
    ...
    h.T(ML+34,H-48,"PHYSIK · KLASSE 10 · FORSCHERHEFT",COP(9),GOLD_D)
...
def build_ebook(src, dst, nav, qrpages=None, seitentexte=None,
    ...
    w._root_object[NameObject("/PageMode")]=NameObject("/UseOutlines")
    w.add_metadata({"/Title":"FeLabs PHYSIK 10 – Forscherheft Realschule NRW",
                    "/Author":"Abdullah Lala",
                    "/Subject":"Kräfte · Bewegung · Energie, Arbeit & Leistung · Kraftwerke, Energieversorgung & Klimaschutz",
                    "/Keywords":"FeLabs, Physik, Klasse 10, Realschule NRW, Forscherheft, Magnetismus, Motor, Induktion, Radioaktivität, Kernenergie, Fassung 1.0, Stand 2026-08-30",
                    "/Creator":"FeLabs"})
...
if __name__=="__main__":
    ...
    out=os.path.join(bd,"FeLabs_Physik_10_Realschule_NRW_Druck%s.pdf"%("_weiss" if WEISS else ""))
    pages[0].save(out,"PDF",resolution=150,save_all=True,append_images=pages[1:])
    ...
    if "--druck" in sys.argv:
        import shutil; ziel=os.path.join(ABLAGE, f"FeLabs_Physik_10_Realschule_NRW_Druck.pdf")
        shutil.copy2(out,ziel); print("Druckfassung zusätzlich:",ziel)
    try:
        eb=os.path.join(ABLAGE, f"FeLabs_Physik_10_Realschule_NRW.pdf")
        try:
            ...
            eb=os.path.join(ABLAGE, f"FeLabs_Physik_10_Realschule_NRW_NEU.pdf")
            build_ebook(out, eb, nav, qrpages, seitentexte, mess_start, mess_starts,
                        toc_pn, bk_start)
