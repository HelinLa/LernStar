...
def book_cover():
    ...
    # eingetragenen LH_KLEIN liefe das Deckblatt sonst unter den Satzspiegel -
    # unsichtbar, weil dort nur eine Grundlinie steht.
...
def lb_testloesungen(t,ch,pn):
    ...
    # haftet=2, nicht len(teilpunkte): Ein Teilpunkt ist jetzt eine LISTE von
    # Zeilenbausteinen, die Zahl der Bausteine also nicht mehr die Zahl der
    # Teilpunkte. Die Ueberschrift haelt die erste Zeile des ersten Teilpunkts -
    # mehr braucht es nicht, damit sie nicht allein am Seitenfuss steht.
...
def build_lehrerband():
    ...
    # gesetzten Band messen, nicht nur den Schuelerteil. Der Lehrerband ist der
    # groessere Teil des Satzes - haette er zu lange Zeilen, saehe man es an
    # einer Probe ueber die Schuelerseiten allein nicht.
...
# beieinander": Eine grosszuegige Schwelle zieht auch Listen zusammen
# (Inhaltsverzeichnis, Ankreuzzeilen, Alltagspunkte). Deren kurze Zeilen gaelten
# dann als "gefuellt" und drueckten den Median weit unter den Fliesstext.
ZEILENTOLERANZ = 1.5
...
def lesezeilen(protokoll):
    ...
    Lauf - und die liest niemand in einem Zug."""
    nach_y={}
...
if __name__=="__main__":
    ...
    if _alle:
        ...
        print("Zeilenprobe: KEINE Zeile gemessen - die Probe hat nichts geprueft.")
