# -*- coding: utf-8 -*-
"""FELO Physik Einfuehrungsphase, gymnasiale Oberstufe NRW: zwei Kapitel mit Test.

Der Satzmotor liegt unveraendert in ../arbeitsheft/build_final.py - er ist fuer Klasse 5
gebaut und gepruefte Ware, deshalb wird er hier nur importiert und nicht kopiert.
Bandspezifisch ist allein plan.py: Kapitel, Themen, Simulationen, Lehrplan.

DER SCHUELERBAND STEHT IM NEUEN SATZ (Fassung "spec", ../arbeitsheft/felo_design.py).
    ...
    aus der Groessentafel des Moduls - eine nackte Zahl wie AV(15.5) gibt es hier
    nicht mehr. Jede gesetzte Zeichenkette laeuft durch die cmap-Pruefung.
...
DREI ENTSCHEIDUNGEN DES AUFTRAGGEBERS SIND HIER UMGESETZT
    ...
    2  Der Loesungsteil ist ein EIGENES PDF: ..._Lehrerband.pdf, mit eigenem
       Deckblatt und eigener Zaehlung, im heutigen kompakten Satz - er ist fuer
       Erwachsene. Vorbild: arbeitsheft8/build_book.py::build_lehrerband.
    3  Der Messwerte-Anhang BLEIBT im Schuelerband. Die Forscherseiten verweisen
       ...
       den Lehrerband wandern, sonst zeigt jede der 23 Seiten ins Leere.

VIER BESONDERHEITEN DER OBERSTUFE, die beim Kopieren aus der Sek I durchrutschen
    1  ANDERE KOMPETENZCODES. S1-S7 / E1-E11 / K1-K10 / B1-B8 (Heft 4721), nicht
       UF/E/K/B. Geladen wird das Modul, das plan.LEHRPLAN nennt.
    2  ANDERE BASISKONZEPTE, und NICHT jedes Inhaltsfeld nennt alle vier:
       Inhaltsfeld 2 fuehrt nur zwei. Die Kapitel- und Basiskonzeptseiten lesen
       sie deshalb aus lehrplan_gost.feld() und zaehlen sie nicht selbst auf.
    3  DIE ANFORDERUNGSBEREICHE STEHEN HIER IM LEHRPLAN (Heft 4721, Kap. 4,
       S. 59-60) - anders als in der Sek I. Die Seite "Ueber dieses Heft" darf
       sie deshalb als Vorgabe des Plans kennzeichnen und tut das auch.
    4  FUENF DATENBLATTSEITEN: ki7, gw6, gw7, gw9 und gw11 haben keine
       Simulation und tragen im Inhalt ein Feld "daten". Bei ihnen heisst
       Abschnitt 3 "Auswerten & beurteilen", der Quellenkasten nennt das
       Datenblatt statt der Simulation, und die Zahlen stehen als gedruckte
...
# von Klasse 5 (bilder.py!) gegen die eigenen, und das Heft liest fremde Bilder ein.
...
# Damit stuenden ab hier arbeitsheft/plan.py und arbeitsheft/bilder.py vor den
# eigenen (Falle 3 in CLAUDE.md). Also zurueckdrehen: MOTOR gehoert ans Ende.
...
_MOD=os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),"arbeitsheft")
import plan
# Welcher Lehrplan gilt, steht in plan.py. Die Oberstufe hat einen eigenen
# (Heft 4721) mit ANDEREN Kompetenzcodes und ANDEREN Basiskonzepten als die
# Sekundarstufe I - wer hier lehrplan.py stehen laesst, druckt Inhaltsfelder und
# Basiskonzepte, die es in der Oberstufe gar nicht gibt.
_LP=getattr(plan,"LEHRPLAN","gym")
_spec=_ilu.spec_from_file_location(f"lehrplan_{_LP}",os.path.join(_MOD,f"lehrplan_{_LP}.py"))
lehrplan=_ilu.module_from_spec(_spec); _spec.loader.exec_module(lehrplan)
_kn="kompetenzen_gost" if _LP=="gost" else "kompetenzen"
_kspec=_ilu.spec_from_file_location(_kn,os.path.join(_MOD,_kn+".py"))
...
# sonst schreibt dieses Heft seine Dateien in den Ordner des anderen.
HERE=_HIER
...
def sim_url(tid):
    ...
    # sie es nicht wissen: m1 und m4 zeigen beide auf "magnetfeld", s2 und s6 beide auf
    # "stromkreis-lampe". Die App blendet damit die richtige Forscherfrage ein.
...
# Kapitel und Themen kommen aus plan.py - eine einzige Quelle.
CHAPTERS=[{"id":k["id"],"title":k["titel"],"acc":k["acc"],
           "topics":[th["id"] for th in k["themen"]]} for k in plan.KAPITEL]
VORHABEN={k["id"]:k["vorhaben"] for k in plan.KAPITEL}
# Die Einfuehrungsphase hat keine Schaltzeichen-Seite und keine Schaltbilder.
SCHALTBILD={}
SZ_NACH=None

def _diag(chid,tid):
    # Dieser Band arbeitet ausschliesslich mit eigenen Einstiegsbildern, nicht mit
    # gezeichneten Szenen. Die Rueckfallebene aus Klasse 5 entfaellt deshalb.
    return None

# Klasse, Schulform und die Dateinamen kommen aus plan.py - so bleibt dieses
# Skript bandunabhaengig und der Dateiname auf dem Schreibtisch stabil.
# GEMISCHTE SCHREIBUNG, keine Versalien: "keine Woerter in Grossbuchstaben" ist
# eine Vorgabe des neuen Satzes, und BANDNAME steht in jeder Kopfzeile.
KL       = str(plan.KLASSE)                         # "11"
SFORM    = plan.SCHULFORM                           # "Gymnasiale Oberstufe NRW"
SFORM_DT = SFORM.replace(" ","_")                   # fuer den Dateinamen
BANDNAME = f"FELO Physik {KL} · {SFORM}"
DATEINAME = f"FELO_Physik_{KL.replace('/','_')}_{SFORM_DT}"
FUSS     = f"FELO Physik {KL} · Forscherheft"
# Ueberschrift des Kapitelverzeichnisses auf dem Deckblatt. Die Baende der Reihe
# haben zwei bis vier Kapitel - "Die vier Kapitel" aus dem Musterband waere hier
# schlicht falsch, dieser Band hat zwei. Gleiche Loesung wie in gym56/gym9.
_ZAHLWORT={1:"Das Kapitel",2:"Die beiden Kapitel",3:"Die drei Kapitel",
           4:"Die vier Kapitel",5:"Die fünf Kapitel",6:"Die sechs Kapitel"}
KAPITELZEILE=_ZAHLWORT.get(len(CHAPTERS),"Die Kapitel dieses Bandes")
# Betreff und Schlagworte des E-Books aus den Kapiteln, nicht von Hand gepflegt
...
# der Satzspiegel des Blattes und traegt ueber 100 Zeichen je Zeile. Gemessen an
# Kapitel 1 lagen 29 % aller gefuellten Lesezeilen darueber, die laengste bei
# 107 Zeichen - genau der Gewinn, den die groessere Schrift bringen sollte.
...
# Dieselbe ZEILE, eine Stufe kleiner gesetzt. 830 Einheiten tragen bei 12 pt rund
# 81 Zeichen, bei 10 pt aber 97 - die Obergrenze des Moduls (85) ist damit
# ueberschritten, ohne dass es jemand merkt: die Zeile sieht ja kuerzer aus.
# GEMESSEN an diesem Band, ueber alle vom Umbruch gefuellten Lesezeilen: vier
# Zeilen ueber 85 Zeichen, Maximum 93 - alle vier die Quellenzeile eines
# Datenblatts. Die Spalte wird deshalb im Verhaeltnis der Schriftgrade
# mitverkleinert, dann traegt sie wieder dieselben 81 Zeichen.
LESE_KLEIN = round(LESE*fd.KLEIN/float(fd.FLIESS))    # 692 Einheiten = 332 pt
...
# Der Code ragt damit 8 Einheiten (3,8 pt) ueber X1 hinaus. Das ist der Preis
# dafuer, dass 23 gedruckte Codes weiter gelesen werden - bewusst so gelassen.
...
# (dort, wo niemand ein Raetsel setzt). In diesem Band findet die Probe sieben:
# AE (astronomische Einheit), GPS, ISS und SI sind echte Abkuerzungen; II und III
# stehen dabei, weil die Anforderungsbereiche im Inhalt als Zeichenketten liegen,
# und NICHT, weil ein Fachtext das Wort zur Hervorhebung gross schreibt. Keines
# der 22 Wortschatzwoerter faellt darunter - sie werden also alle klein gesetzt.
...
# Die frueher hier stehende Umgehung verklammern() ist ins Modul gewandert:
# fd.umbrechen() rechnet die Klammerung seit dem 09.09.2026 selbst durch
# (fd.verklammern), damit alle Baende der Reihe sie haben und nicht jeder sie
# neu erfindet. Hier steht sie deshalb nicht mehr - zwei Fassungen derselben
# Regel waeren genau die Sorte Fehler, die sie verhindern soll.
...
def einpassen(bauer,nmin,nmax):
    ...
    meisten Schreibzeilen. Grund fuer die erste: Mit acht Zeilen je Weiterdenken-
    Aufgabe rutschte eine einzige Linie auf ein zweites Blatt - Seite 32 trug
    nichts als die Fortsetzungszeile und einen Strich, viermal im Band. Mit
    sieben passt die Einheit auf ein Blatt.

    fd.schreibraum_auffuellen kann das nicht leisten: es legt Restplatz nur
    HINTER dem letzten Schreibblock nach (die letzte Aufgabe bekaeme elf Zeilen,
    die davor je zwei) und macht eine Einheit nie kuerzer."""
    mess=[]
...
def setze_einheit(B,kopf,titel,pn,name,qr=None,fuellen=False,kopf_rechts=None,
    ...
    PIL den Rest abschneiden (so lief ch_uebung bisher unter die Blattkante).

    `ausgleich` fuer Einheiten, bei denen HINTER dem letzten Schreibblock noch
    etwas steht - Selbst-Check und Ueberleitung auf der Uebungsseite, Notenspiegel
    und Zuspruch im Test. Dort kann fd.schreibraum_auffuellen den Rest der letzten
    Seite nicht holen: es legt Linien nur VOR diesem Schlussstueck nach, und das
    Schlussstueck bleibt allein auf der letzten Seite stehen. Gemessen an diesem
    Band: Uebungsseite gw8 endete auf Seite 3 bei 598 von 1674 Einheiten (30 %),
    beide Kapiteltests bei 415 (18 %) - ein Blatt fuer den Notenspiegel. Der
    Ausgleich verteilt denselben Satz auf dieselbe Seitenzahl, aber gleichmaessig."""
    ...
    # Gefragt wird die DURCHGERECHNETE Reichweite, nicht die genannte: haelt ein
    # Baustein nur mittelbar (ueber den, den er haelt), faellt er sonst durch.
...
def gitterzeichen(d,cx,cy,r,fill):
    ...
    Kaestchen (frueher build_book.py:357). Gezeichnet kann das nicht passieren."""
...
def datenblatt(h,d,y,daten):
    ...
    FUENF Seiten dieses Bandes haben keine Simulation und tragen stattdessen
    gedruckte Zahlen: ki7, gw6, gw7, gw9, gw11. Alle fuenf Blaetter sind
    dreispaltig. Die Tabelle steht wie jede andere auf der Lesespalte
    (fd.KASTEN_X0..fd.KASTEN_X1) - ueber die volle Blattbreite truege sie gut
    100 Zeichen je Zeile.

    Zellen brechen UM, sie werden nicht verkleinert: das erledigt fd.tabelle.
    Gemessen an allen fuenf Datenblaettern (Spalten 267/250/217 Einheiten) passt
    kein einzelnes Wort ueber seine Spalte hinaus; die laengsten sind
    "Navigationssatellit" (194), "Erdmittelpunkt" (172) und "Verlaesslichkeit"
    (162). Das Sicherheitsnetz des alten Satzes (Schrift stufenweise kleiner
    ziehen) wird also nicht gebraucht und faellt weg."""
    n=len(daten["spalten"])
...
#  FORSCHERSEITE
# ═════════════════════════════════════════════════════════════════════════════
def topic_pages(cfg,chtitel,fno,pn):
    ...
    dat=cfg.get("daten")
    am_schirm=tid in plan.AM_BILDSCHIRM
    B=[]
    ...
    if cfg.get("stufe")=="vertiefung": kopf+=" · Forscherwissen"
...
def b_richtigfalsch_kopf(n_aussagen,labels=("richtig","falsch")):
    """Die Spaltenkoepfe "richtig" und "falsch" - und ALLE Aussagen darunter.

    Der Musterband haelt nur die erste Aussage fest. Bricht der Block dann um,
    stehen auf der Folgeseite Kaestchen ohne Beschriftung: zwei leere Quadrate,
    von denen niemand mehr weiss, welches "richtig" heisst. Gemessen an diesem
    Band traf das drei Uebungsseiten und BEIDE Kapiteltests. Der Kopf haelt
    deshalb den ganzen Block. Preis: gemessen null Seiten - der Block ist
    kleiner als ein Blatt und wandert nur als Ganzes."""
    def f(h,d,y):
    ...
    return bst("Spaltenkopf",f,abstand=4,haftet=max(1,n_aussagen))
...
def b_mc(frage,optionen):
    """Frage und ALLE Antwortmoeglichkeiten gehoeren auf dieselbe Seite.

    Der Musterband haelt nur die erste Antwort fest (haftet=1). Das genuegt dort;
    in diesem Band liegt der Multiple-Choice-Block genau auf der Blattkante der
    Uebungsseite: gemessen an allen 28 Uebungsseiten stand bei 26 von ihnen die
    letzte Antwortmoeglichkeit allein auf der Folgeseite. Wer ankreuzen soll,
    muss aber alle Antworten nebeneinander sehen; sonst vergleicht er die dritte
    aus dem Gedaechtnis. Die Frage haelt deshalb so viele Bausteine fest, wie es
    Antworten gibt. Preis: gemessen 58 -> 60 Uebungsseiten, also zwei Blatt."""
    B=[b_para(frage,art="med",breite=LESE,haftet=len(optionen),name="MC-Frage")]
    for o in optionen:
...
def ch_uebung(ub,cfg,chtitel,fno,pn,ueber=None):
    ...
    return setze_einheit(B,f"{chtitel} · Forscherkreis {fno:02d}","Übungen",pn,
                         "Übungsseite "+str(ub["id"]),fuellen=True,ausgleich=True)
...
def ch_testprep(prep,words,chtitel,pn):
    def bauer(nl):
        ...
        return B
    # Der Schreibraum der vier Selbstprüfungsfragen waechst gleichmaessig in den
...
def ch_test(test,chtitel,pn):
    ...
    def b_kopfzeile(h,d,y):
        ...
        yy=y+12; xx=fd.X0+20
        for lab,br in (("Name:",340),(f"Klasse: {KL}",0),("Datum:",200)):
    ...
    B.append(b_richtigfalsch_kopf(len(test["a2"])))
...
# nicht nachgerechnet. Frueher wurden die Klickflaechen starr bei y=700 gesetzt
# und lagen daneben, sobald ein Kapitel ein Vorhaben bekam.
TRENNBOXEN={}
...
def divider(num,ch,pn):
    ...
    if _fd:
        ...
        B.append(b_para("Basiskonzepte: "+" · ".join(_bk.keys()),grad=fd.KLEIN,
                        lh=fd.LH_KLEIN,breite=LESE_KLEIN,abstand=fd.ABS_ABSCHNITT,
                        name="Basiskonzepte"))
    ...
    # Eine Ueberschrift, unter der genau EIN Kaestchen steht, ist keine Liste.
    # Ohne diese Zeile blieb bei Kapitel 3 (14 Themen) das erste Kaestchen allein
    # am Fuss der ersten Seite stehen und die uebrigen dreizehn standen dahinter.
    B[-1].haftet=3
...
def book_cover():
    ...
    y+=24
    fd.T(h,fd.X0,y,"Physik "+KL,fd.schrift("bold",DISPLAY_MITTEL),fd.STIL["h1"])
    y+=LH_DISPLAY+6
    ...
    fd.T(h,fd.X0,y,KAPITELZEILE,fd.schrift("bold",fd.KASTEN_KOPF),fd.STIL["akzent"])
    h.ln([(fd.X0,y+LH_KOPF+2),(fd.X1,y+LH_KOPF+2)],fd.STIL["zart"],1.2)
    y+=LH_KOPF+22
...
def about_pages(pn):
    ...
    B.append(b_para("Dieses Heft nimmt Jugendliche als Forscherinnen und Forscher ernst. Statt fertige Antworten "
    ...
    pillars=[
     ...
     ("Nah am Alltag","Alltagssituationen und Schülersprache; die hervorgehobene Forscherfrage könnten die Jugendlichen selbst gestellt haben."),
    ...
    # HIER ist der Unterschied zur Sekundarstufe I: Die Anforderungsbereiche
    # stehen im Oberstufenplan AUSDRUECKLICH (Heft 4721, Kap. 4, S. 59-60), samt
    ...
    B.append(b_para("Das Werk und seine Teile sind urheberrechtlich geschützt. Jede Verwertung außerhalb der engen "
        ...
        grad=fd.KLEIN,lh=fd.LH_KLEIN,breite=LESE_KLEIN,name="Rechte"))
...
def basiskonzept_seiten(pn):
    ...
    for ch in CHAPTERS:
        ...
        # (151 Einheiten). Die Oberstufe hat "Mathematisieren und Vorhersagen"
        # und "Zufall und Determiniertheit": gemessen 176 und 174 Einheiten fuer
        # das laengste Wort allein - beide liefen ueber die Spaltenlinie in die
        # Stichworte hinein. Mit 0,28 sind es 200 Einheiten. h.wrap trennt nur an
        # Leerzeichen, ein zu langes EINZELWORT kann es also nicht retten.
    ...
    B.append(b_para("Die Zuordnung eines Themas zu einem Basiskonzept trifft der Verlag anhand der Stichworte "
        ...
        lh=fd.LH_KLEIN,breite=LESE_KLEIN,name="Fussnote"))
...
def messwerte_pages(start_pn):
    ...
    # EINE durchlaufende Einheit, nicht vier. Kapitelweise gesetzt trug die
    # letzte Seite von Kapitel 2 einen einzigen Eintrag (15 % gefuellt) - ein
    # Anhang darf durchlaufen, die Kapitel bekommen eine Zwischenueberschrift.
    ...
    for ci,ch in enumerate(CHAPTERS):
        ...
        # Datenblatt-Seiten haben ihre Werte ohnehin auf der Seite selbst stehen.
...
#  (gemessen 90 Seiten in der Reihe, hier 8). Die Funktionen und assessment.json
#  bleiben unangetastet, damit die Woerter weiter die Test-Vorbereitung tragen
#  und die Seiten jederzeit wieder gesetzt werden koennen. Sie stehen im ALTEN
#  Satz - wer sie zurueckholt, muss sie vorher auf felo_design umstellen.
# ═════════════════════════════════════════════════════════════════════════════
def footer(h,pn,acc):
    h.circ(ML+8,H-40,13,outline=GOLD,w=2); h.circ(ML+8,H-40,4,fill=GOLD)
    h.T(ML+34,H-48,f"PHYSIK · KLASSE {KL} · FORSCHERHEFT",COP(9),GOLD_D)
...
def ch_kreuzwort(entries,name,pn,acc):
    ...
    cell=min(60,int((W-2*ML-40)/nc),int(620/max(nr,1))); gw=nc*cell; gh=nr*cell; gx=(W-gw)/2; gy=252
    ...
    def _hoehe(gr,lh,ab):
        return max(30+sum(len(h.wrap(c2,AV(gr),cw-30))*lh+ab for _,_,c2 in sp) for sp in (across,down))
    gr,lh,ab=13.5,21,11
    while gr>9.5 and cy0+_hoehe(gr,lh,ab)>H-96:
        gr-=0.5; lh=max(15,int(round(gr*1.55))); ab=max(5,ab-1)
    def cl(ci,title,items,yy):
        h.tracked(colx[ci],yy,title,COP(13),GOLD_D,3,center=False); yy+=30
        for n2,w,c2 in items:
            h.T(colx[ci],yy,f"{n2}.",AVB(gr),INK); yy=h.para(colx[ci]+30,yy,c2,AV(gr),INK,cw-30,lh)+ab
        return yy
...
# ueber dem Text: eine Zeile mit dem Inhaltsfeld, darunter die Basiskonzepte.
#
# GEMESSEN, NICHT GESCHAETZT. In der Sekundarstufe I heissen die Basiskonzepte
# "Energie" oder "System" und ihre Stichworte sind kurz - zwei Zeilen, also die
# feste Zahl 56. Die Oberstufe nennt je Konzept einen ganzen Satz: In Kapitel 1
# dieses Bandes sind es vier Konzepte und FUENF Zeilen. Mit der festen 56 lief
# der Block quer durch die Ueberschrift der ersten Loesung ("Fünf Stopps, eine
# Steigung" stand mitten im Kernlehrplan-Text). Die Hoehe wird deshalb aus dem
# echten Umbruch geholt.
_LEHRPLAN_LH = 17          # Zeilenabstand des Basiskonzept-Absatzes
_LEHRPLAN_LUFT = 22        # Luft zwischen Block und erster Loesung


def _lehrplan_text(chid):
    _fd=lehrplan.feld(chid)
    if not _fd: return None
    _nr,_nm,_bk=_fd
    return _nr,_nm," · ".join(f"{_k}: {_v}" for _k,_v in _bk.items())


def _lehrplan_hoehe(h,chid):
    """Wie hoch wird der Kernlehrplan-Block WIRKLICH? 0, wenn es keinen gibt."""
    t=_lehrplan_text(chid)
    if not t: return 0
    return len(h.wrap(t[2],AV(12),W-2*ML))*_LEHRPLAN_LH+_LEHRPLAN_LUFT
...
def _loes_forscherseiten(ch,ci,pn0):
    ...
    _lh0=_lehrplan_hoehe(h,ch["id"])
    teile=[]; akt=[]; y=214+_lh0
    for i,tid in enumerate(ch["topics"]):
        hh=hoehe(tid)
    ...
    for j,idx in enumerate(teile):
        ...
        _lp=_lehrplan_text(ch["id"])
        if _lp and j==0:
            _nr,_nm,_bktext=_lp
            h.T(ML,196,f"Kernlehrplan: Inhaltsfeld {_nr} · {_nm}",AVM(13),GOLD_D)
            h.para(ML,214,_bktext,AV(12),SUB,W-2*ML,_LEHRPLAN_LH)
        y=214+(_lh0 if (_lp and j==0) else 0)
        for k,i in enumerate(idx):
            ...
            _xr=W-ML
            for _b in ["S","E","K","B"]:   # Oberstufe: Sachkompetenz statt UF
...
def lb_cover():
    ...
    h.tracked(W/2,330,"PHYSIK "+KL,DIDOT(46),INK,4)
    h.tracked(W/2,410,SFORM.upper(),COP(16),GOLD_D,6); h.orn(W/2,452,150)
    z=["Lösungen zu allen %d Forscherseiten"%sum(len(c["topics"]) for c in CHAPTERS),
       "Lösungen der Übungsseiten und Kapiteltests",
...
def build_lehrerband():
    ...
    try:
        ...
        w.add_metadata({"/Title":f"FELO PHYSIK {KL} – {SFORM} – Lehrerband",
                        "/Author":"Abdullah Lala","/Creator":"FELO",
                        "/Subject":f"Lösungen und Erwartungshorizonte zum Forscherheft {SFORM}, Klasse {KL}"})
...
def build_ebook(src, dst, nav, qrpages=None, seitentexte=None,
    ...
    w._root_object[NameObject("/PageMode")]=NameObject("/UseOutlines")
    w.add_metadata({"/Title":f"FELO PHYSIK {KL} – Forscherheft {SFORM}",
                    "/Author":"Abdullah Lala",
                    "/Subject":FACHGEBIETE,
                    "/Keywords":f"FELO, Physik, Klasse {KL}, {SFORM}, Forscherheft, {INHALTSFELDER}, {FACHGEBIETE}, Fassung 1.1",
                    "/Creator":"FELO"})
...
if __name__=="__main__":
    ...
    # Kaestchen wie das frueher gesetzte "▤" (U+25A4) kann so nicht mehr
    # unbemerkt auf die Seite kommen.
    _fehlt={}
    for _s,_art,_gr in fd.GESETZT:
        for _c in fd.fehlende_zeichen(_s,_art): _fehlt.setdefault(_c,0)
        for _c in fd.fehlende_zeichen(_s,_art): _fehlt[_c]+=1
    ...
    import re as _re
    _erlaubt=set(ABKUERZUNGEN)|{"FELO","NRW","QR"}
    _vers={}
    for _s,_a,_g in fd.GESETZT:
        # Nur ganze Versalwoerter. Ohne die beiden Ausschluesse meldet die Probe
        # "QR-C" aus "QR-Code" und "LED-L" aus "LED-Lampe" - zwei Fehlalarme,
        # die beim ersten Lauf tatsaechlich kamen.
    ...
    # ── Kompetenzprobe: DIESER Band hat andere Codes als die Sekundarstufe I. ──
    # Gedruckt werden sie an drei Stellen: im Kaertchen der Abschnittsmarke, an
    # den vier Uebungsaufgaben und bei den Weiterdenken-Aufgaben. Ein aus der
    # Sek I mitgeschleppter Code (UF1) faellt sonst niemandem auf - er sieht aus
    # wie ein Code und ist keiner. kompetenzen_gost.pruefe() haelt sie gegen die
    # 36 Erwartungen des Plans.
    _codes=[]
    for _o in FSD.values():
        _codes+= [c for c in [(_o.get("aufgabe") or {}).get("komp"), _o.get("alltagKomp")] if c]
    for _u in UEBD.values(): _codes+=[c for c in (_u.get("komp") or []) if c]
    for _lst in TRANSFER.values(): _codes+=[a["komp"] for a in _lst if a.get("komp")]
    _codes+=["E1","E3","E5","E6","K3"]          # die festen Codes der Abschnittsmarken
    _falsch=sorted(set(kompetenzen.pruefe(_codes)))
    print("Kompetenzprobe: %d gedruckte Codes gegen %s – %s"
          % (len(_codes), _kn,
             "alle gueltig." if not _falsch else "UNGUELTIG: "+" ".join(_falsch)))
    print("   benutzt: "+" ".join(sorted(set(_codes),key=lambda c:(c[0],int(c[1:])))))

    out=os.path.join(bd,DATEINAME+"_Druck%s.pdf"%("_weiss" if WEISS else ""))
    pages[0].save(out,"PDF",resolution=150,save_all=True,append_images=pages[1:])
