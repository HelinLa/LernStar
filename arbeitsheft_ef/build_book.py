# -*- coding: utf-8 -*-
"""FeLabs Physik Einfuehrungsphase, gymnasiale Oberstufe NRW: zwei Kapitel mit Test.
...
ABLAGE = os.path.expanduser("~/Desktop/Projekt KI Verkauf Physik /Gymnasiale Oberstufe")
...
BANDNAME = f"FeLabs Physik {KL} · {SFORM}"
DATEINAME = f"FeLabs_Physik_{KL.replace('/','_')}_{SFORM_DT}"
FUSS     = f"FeLabs Physik {KL} · Forscherheft"
# Der Lehrerband ist ein eigenes Heft und traegt deshalb eine eigene Fusszeile.
FUSS_LB  = f"FeLabs Physik {KL} · Lehrerband"
...
def about_pages(pn):
    ...
    # Gekuerzt am 18.09.2026: Der FeLabs-Lernweg braucht zwei Buchstaben mehr, und
    # das Vorwort nennt ihn jetzt ausdruecklich. Ohne diese Kuerzung lief die
    # Seite ueber - und 27 Seitenzahlen des Bandes waeren gewandert.
    B.append(b_para("Dieses Heft nimmt Jugendliche als Forscherinnen und Forscher ernst: Sie vermuten zuerst "
        "selbst, prüfen ihre Vermutung und formulieren die Erkenntnis danach in eigenen Worten.",
        breite=LESE,abstand=fd.ABS_ABSCHNITT,name="Vorwort"))
...
def lehrer_bloecke(tid):
    ...
    if codes:
        L.append(("Kompetenzen dieser Einheit",
                  ["%s – %s"%(c,kompetenzen.KOMPETENZ.get(c,"")) for c in codes]))
    return L
...
def lb_cover():
    ...
    h.ln([(fd.X0,y),(fd.X1,y)],fd.STIL["akzent"],1.6)
    y+=24
    fd.T(h,fd.X0,y,"Physik "+KL+" · Forscherheft",fd.schrift("bold",fd.HAUPT),fd.STIL["h1"])
    y+=LH_HAUPT+8
...
def build_lehrerband(gesetzt=None):
    ...
    try:
        ...
        w.add_metadata({"/Title":f"FeLabs PHYSIK {KL} – {SFORM} – Lehrerband",
...
def build_ebook(src, dst, nav, qrpages=None, seitentexte=None,
    ...
    w._root_object[NameObject("/PageMode")]=NameObject("/UseOutlines")
    w.add_metadata({"/Title":f"FeLabs PHYSIK {KL} – Forscherheft {SFORM}",
                    "/Author":"Abdullah Lala",
                    "/Subject":FACHGEBIETE,
                    "/Keywords":f"FeLabs, Physik, Klasse {KL}, {SFORM}, Forscherheft, {INHALTSFELDER}, {FACHGEBIETE}, Fassung 1.1",
                    "/Creator":"FeLabs"})
...
if __name__=="__main__":
    ...
    for ci,ch in enumerate(CHAPTERS):
        ...
        for ti,tid in enumerate(ch["topics"]):
            ...
            qrpages[pn]=tid
            HEFTSEITE[tid]=pn      # fuer den Seitenverweis im Lehrerteil
    ...
    import re as _re
    _erlaubt=set(ABKUERZUNGEN)|{"FeLabs","NRW","QR"}
    _vers={}
    ...
    print("")
    # `_lb` wurde oben schon gesetzt, damit die Proben es messen konnten -
    # hier wird es nur noch geschrieben.
