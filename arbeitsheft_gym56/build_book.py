# -*- coding: utf-8 -*-
"""FELO PHYSIK · Gymnasium NRW (G9): Kapitel mit Test dahinter.

Klasse und Schulform kommen aus plan.py - diese Datei ist fuer ALLE FUENF
Gymnasialbaende dieselbe (gym56, gym7, gym8, gym9, gym10) und liegt in allen
fuenf Ordnern identisch. Beim naechsten Band nur plan.py schreiben und die
Skripte kopieren. gym56 traegt die Klasse "5/6" - eine Zeichenkette, kein
Zahlwert; gym9 und gym10 haben Datenblattseiten (Themen ohne Simulation).
...
Klassenspezifisch ist allein plan.py: Kapitel, Themen, Videos, Simulationen.
...
DREI ENTSCHEIDUNGEN DES AUFTRAGGEBERS SIND HIER UMGESETZT
    ...
    2  Der Loesungsteil ist ein EIGENES PDF: ..._Lehrerband.pdf, mit eigenem
       ...
       Erwachsene. Vorbild: arbeitsheft_foe8/build_book.py::build_lehrerband.
...
_spec=_ilu.spec_from_file_location("lehrplan_gym", os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "arbeitsheft", "lehrplan_gym.py"))
...
# Klasse und Schulform stehen in plan.py - damit ist dieses build_book.py fuer
# ALLE Gymnasialbaende gleich und muss beim naechsten Band nicht angefasst werden.
KL       = str(plan.KLASSE)                          # "5/6", "7", "8", "9", "10"
SFORM    = plan.SCHULFORM                            # "Gymnasium NRW"
SFORM_GR = SFORM.upper()                             # "GYMNASIUM NRW"
SFORM_DT = SFORM.replace(" ", "_")                   # "Gymnasium_NRW"
# Betreff und Schlagworte aus den Kapiteln des Bandes, nicht von Hand gepflegt
...
# Die Gymnasialreihe hat keine Schaltzeichen-Seite und keine eingebauten Schaltbilder.
SCHALTBILD={}
SZ_NACH=None

def _diag(chid,tid):
    # Die Gymnasialreihe arbeitet ausschliesslich mit eigenen Einstiegsbildern,
    # nicht mit gezeichneten Szenen. Die Rueckfallebene aus Klasse 5 entfaellt.
    return None

BANDNAME=f"FELO Physik {KL} · {SFORM}"
DATEINAME=f"FELO_Physik_{KL.replace('/', '_')}_{SFORM_DT}"   # 5/6 darf keinen Pfad ergeben
FUSS=f"FELO Physik {KL} · Forscherheft"
# Ueberschrift des Kapitelverzeichnisses auf dem Deckblatt. Die Baende haben
# zwei bis vier Kapitel - eine feste Zahl im Text waere in vier von fuenf
# Baenden falsch, und falsch gedruckt faellt es niemandem mehr auf.
...
# dafuer, dass 25 gedruckte Codes weiter gelesen werden - bewusst so gelassen.
...
# (dort, wo niemand ein Raetsel setzt). Welche das sind, misst die Versalprobe
# am Ende des Laufs - die Liste steht nicht im Code, sie wird gelesen.
...
def setze_einheit(B,kopf,titel,pn,name,qr=None,fuellen=False,kopf_rechts=None):
    ...
    PIL den Rest abschneiden (so lief ch_testprep bisher unter die Blattkante)."""
    ...
    if fuellen:
        ...
        # Und danach noch einmal fuer die Alltag-Linien. Grund: der Nachschlag
        # legt Zeilen immer hinter den LETZTEN Schreibblock seines Praefixes.
        # Auf einer Forscherseite steht danach aber noch der Alltag-Block - faellt
        # der als Letztes auf ein neues Blatt (Thema wa13: laengster Fachtext des
        # Bandes), landet der Nachschlag auf der vollen Seite davor, und die neue
        # bleibt zu zwei Dritteln weiss. Beide Aufrufe nehmen sich zurueck, sobald
        # sie eine Seite kosten wuerden; Einheiten ohne Alltag-Linien merken nichts.
        B,hoehen,seiten=fd.schreibraum_auffuellen(B,hoehen,seiten,fd.STIL,
...
def b_schreibzeile(label,abstand=fd.ABS_ZEILE,name=None):
    ...
    Verallgemeinert das frühere b_begruende: "Begründe:", "Eigene Vermutung:"
    und "Das sehe ich an diesem Wert:" sind derselbe Baustein. Die Linie setzt
    hinter der GEMESSENEN Beschriftungsbreite an, nicht hinter einem Schätzwert -
    "Eigene Vermutung:" ist doppelt so breit wie "Begründe:"."""
    def f(h,d,y):
...
def b_kreuzreihe(vorsatz,labels,abstand=fd.ABS_ZEILE,name="Kreuzreihe"):
    ...
    Für "Meine Vermutung war: ☐ richtig ☐ teilweise richtig ☐ nicht richtig".
    Untereinander kostete das drei Zeilen, die hier bei den Schreiblinien fehlen
    ([[platz-auf-der-seite-messen]]). Jede Gruppe wird EINZELN vermessen und
    fortlaufend gesetzt; passt die Reihe nicht in die Spalte, bricht sie um,
    statt unter dem Satzspiegel zu verschwinden."""
    def f(h,d,y):
        ft=fd.schrift("med",fd.FLIESS)
        ...
        for lab in labels:
            ...
            if x+br>fd.X0+fd.SPALTE and x>fd.X0:      # Umbruch statt Überlauf
                zeile+=fd.LH; x=fd.X0
...
def datenblatt(h,d,y,daten):
    ...
    In der Gymnasialreihe tragen gym9 und gym10 je eine solche Seite; in 5/6, 7
    und 8 hat jedes Thema eine Simulation. Der Renderer steht deshalb in allen
    fuenf Baenden gleich da. Zellen werden UMGEBROCHEN, nicht verkleinert -
    fd.tabelle misst die Hoehe jeder Reihe an ihrer hoechsten Zelle."""
    n=len(daten["spalten"])
...
def topic_pages(cfg,chtitel,fno,pn):
    ...
    Die Zahl der Schreibzeilen in Abschnitt 5 wird EINGEPASST, nicht gesetzt.
    Gemessen an diesem Band: mit den vier Zeilen aus der Inhaltsdatei rutschten
    bei fuenf Themen die letzten drei Linien und der Alltag-Block auf ein drittes
    Blatt, das danach zu 69 Prozent leer blieb. Mit drei Zeilen passt die Einheit
    auf zwei Seiten - und schreibraum_auffuellen legt den frei gewordenen Platz
    unten wieder als Linien nach. Der Schueler bekommt so MEHR Schreibraum auf
    WENIGER Seiten. Dieselbe Regel wie in ch_testprep und ch_transfer."""
    tid=cfg.get("id")
    ...
    def bauer(nl):
        ...
        # ① Das Problem
        #
        # OHNE Einstiegsbild (Konzeptentscheidung Abdullah, 12.09.2026). Das Bild
        # sass in der RANDSPALTE (922..1168), der Text in der Lesespalte
        # (72..902) - sein Wegfall schafft also KEINEN Platz fuer die neuen
        # Abschnitte, sondern raeumt die Randspalte frei. Gemessen, nicht
        # geschaetzt. Nebenwirkung: die offenen Bildplatzhalter mehrerer Baende
        # sind damit erledigt.
        ...
        #
        # Vorher waren es zwei (predict[:2]); zwei Moeglichkeiten sind faktisch
        # eine Ja/Nein-Frage und dazu noch leicht zu erraten. Die dritte muss
        # eine ECHTE Schuelervorstellung sein, keine Fuellantwort. Wo ein Thema
        # erst zwei hat, druckt die Seite zwei - kein Platzhaltertext.
        ...
        # ③ Probiere es aus
        #
        # E4 "Untersuchung und Experiment", nicht E5. Der Code stand auf E5,
        # weil die Datei aus der REALSCHULreihe stammt - dort heisst E5
        # "Untersuchungen und Experimente durchfuehren". Im Gymnasialplan
        # (Heft 3411) heisst E5 "Auswertung und Schlussfolgerung" und E6
        # "Modell und Realitaet". Derselbe Code, eine andere Bedeutung.
        titel3="Auswerten & beurteilen" if dat else (
               "Probiere es in der Simulation aus" if am_schirm else "Probiere es aus")
        B.append(b_marke(3,titel3,"E5" if dat else "E4"))
        if dat or am_schirm:
        ...
        # ④ Meine Beobachtung
        #
        # E2 "Beobachtung und Wahrnehmung". Vorher K3 - im Gymnasialplan heisst
        # K3 aber "Praesentation"; hier wird nichts praesentiert, hier wird
        # eingetragen.
        B.append(b_marke(4,"Meine Beobachtung","E2"))
        # Einspaltige Sicherungstabelle: die Formregeln erlauben 1-2 Spalten, und
        # 14 Seiten des Bandes 5/6 brauchen nur EINE Antwortspalte. Der Kopf der
        # zweiten Spalte bleibt dann leer - die Spalte selbst nicht, dort traegt
        # der Schueler ein. Ohne diese Ergaenzung stuende die Zeile ohne Schreibfeld.
        ...
        # Kuratiert je Thema; sonst ein allgemeiner, der immer traegt.
        for i,sa in enumerate(cfg.get("satzanfaenge") or
                              ["Ich beobachte, dass …"]):
        ...
        #
        # DER SCHRITT, DEN ES IN KEINEM DER 19 BAENDE GAB. Ohne ihn kreuzt das
        # Kind in ② etwas an und kommt nie darauf zurueck - das Raten bleibt
        # folgenlos, und genau das Raten ist der Grund, warum man vorher raten
        # laesst. E5 "Auswertung und Schlussfolgerung".
        #
        ...
        B.append(b_marke(5,"Überprüfe deine Vermutung","E5"))
        ...
        # Aufgabe, die an Sprachhilfe und Schreiblinien haengt - eine einzige
        # unteilbare Gruppe, die auf einer angefangenen Seite nie mehr Platz
        # findet. Gemessen: die untere Haelfte von Seite 2 blieb leer, und die
        # Einheit wuchs von zwei auf drei Blaetter. Die Ueberschrift ⑥ bleibt
        # trotzdem beim Kasten, weil b_marke selbst klammert.
    ...
    B,_nl=einpassen(bauer,3,max(int(auf.get("zeilen",3)),3))
    ...
    # G/E-Differenzierung (APO-SI): am Gymnasium ist KURSE in allen Baenden False,
    # die Zeile bleibt also stumm. Sie steht hier, weil dieselbe Datei in fuenf
    # Ordnern liegt und ein spaeterer Band sie brauchen kann. Klein geschrieben:
    # "E-KURS" waere ein Wort in Grossbuchstaben und faellt durch die Versalprobe.
    if cfg.get("kurs")=="E": kopf+=" · E-Kurs"
...
def ch_test(test,chtitel,pn):
    ...
    def b_kopfzeile(h,d,y):
        ...
        yy=y+12; xx=fd.X0+20
        for lab,br in (("Name:",340),("Klasse: "+KL,0),("Datum:",200)):
...
def about_pages(pn):
    ...
    # Wortlaut des GYMNASIALEN Kernlehrplans (Heft 3411): vier Bewertungscodes,
    # E1–E7 und K1–K4. Die Realschulreihe hat hier andere Zahlen - wer sie
    # uebernimmt, druckt eine Vorgabe, die es fuer diese Schulform nicht gibt.
    ...
    B.append(b_para("Er steht neben dem Kompetenzcode. Die drei Stufen folgen den Bildungsstandards; "
        "der Kernlehrplan Physik des Gymnasiums kennt sie nicht.",art="med",
...
# ═════════════════════════════════════════════════════════════════════════════
def _merksatz_ganz(m):
    """Lueckensatz zum vollen Satz zusammensetzen - MIT Fuge.

    Der Schuelerband setzt Vorsatz, Luecke und Nachsatz mit Abstand; im
    Lehrerband stehen sie hintereinander, und dort entscheidet ein Leerzeichen.
    Die Inhaltsdateien der Gymnasialreihe schreiben den Nachsatz OHNE
    fuehrendes Leerzeichen ("gemessen und sind gleich groß."), die der
    Realschule MIT (" Ladung."). Stur zusammengeklebt stand im Lehrerband
    "werden immer zum Lotgemessen" - in Band 7 bei 47 von 64 Merksaetzen, in
    5/6 bei 51 von 76. Vor einem Satzzeichen bleibt die Fuge weg."""
    post=m.get("post") or ""
    fuge="" if (not post or post[0] in " ,.;:!?)") else " "
    return m["pre"]+" "+m["loesung"]+fuge+post
...
def _loes_bloecke(o):
    return [("VERMUTUNG",o["predict"][o.get("predictOk",0)]),
            ...
            ("SICHERUNG"," ".join(_merksatz_ganz(m) for m in o["merksatz"]))]
...
def lb_cover():
    ...
    h.tracked(W/2,410,SFORM_GR,COP(16),GOLD_D,6); h.orn(W/2,452,150)
    z=[f"Lösungen zu allen {sum(len(c['topics']) for c in CHAPTERS)} Forscherseiten",
...
def build_lehrerband():
    ...
    try:
        ...
        w.add_metadata({"/Title":f"FELO PHYSIK {KL} – {SFORM} – Lehrerband",
                        ...
                        "/Subject":f"Lösungen und Erwartungshorizonte zum Forscherheft Klasse {KL}"})
...
def build_ebook(src, dst, nav, qrpages=None, seitentexte=None,
    ...
    w.add_metadata({"/Title":f"FELO PHYSIK {KL} – Forscherheft {SFORM}",
                    ...
                    "/Subject":FACHGEBIETE,
                    "/Keywords":f"FELO, Physik, Klasse {KL}, {SFORM}, Forscherheft, {INHALTSFELDER}, {FACHGEBIETE}, Fassung 1.0",
                    "/Creator":"FELO"})
