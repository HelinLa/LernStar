...
def _loes_uebungsseiten(ch,ci,pn0):
    """Loesungen zu Uebung, Kapiteltest und Wortschatz - zweispaltig, ueber mehrere Seiten."""
    im,d=newp(CREAM); h=hp(im,d)                     # nur zum Ausmessen
...
def lb_cover():
    ...
    n_fs=sum(len(c["topics"]) for c in CHAPTERS)
    im,d=newp(CREAM); h=hp(im,d)
    ...
    h.tracked(W/2,330,"PHYSIK 5/6",DIDOT(46),INK,4)
    h.tracked(W/2,410,"REALSCHULE NRW",COP(16),GOLD_D,6); h.orn(W/2,452,150)
    z=[f"Lösungen zu allen {n_fs} Forscherseiten","Lösungen der Übungsseiten und Kapiteltests",
...
def build_lehrerband():
    ...
    try:
        ...
        w.add_metadata({"/Title":"FELO PHYSIK 5/6 – Realschule NRW – Lehrerband",
                        "/Author":"Abdullah Lala","/Creator":"FELO",
                        "/Subject":"Lösungen und Erwartungshorizonte zum Forscherheft Klasse 5/6"})
