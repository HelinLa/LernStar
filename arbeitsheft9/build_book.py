# -*- coding: utf-8 -*-
"""Physik-Gesamtheft Klasse 9 Realschule: vier Kapitel, Test hinter jedem Kapitel.
...
DREI ENTSCHEIDUNGEN DES AUFTRAGGEBERS SIND HIER UMGESETZT
    ...
    3  Der Messwerte-Anhang BLEIBT im Schuelerband. Die Forscherseiten verweisen
       ...
       den Lehrerband wandern, sonst zeigt jede der 42 Seiten mit Simulation
       ins Leere.
"""
...
ABLAGE = os.path.expanduser("~/Desktop/Projekt KI Verkauf Physik /Arbeitshefte Realschuhe PH")
...
# Klasse 9 hat keine Schaltzeichen-Seite und keine eingebauten Schaltbilder.
SCHALTBILD={}
SZ_NACH=None

def _diag(chid,tid):
    # Klasse 9 arbeitet ausschliesslich mit eigenen Einstiegsbildern, nicht mit
    # gezeichneten Szenen. Die Rueckfallebene aus Klasse 5 entfaellt deshalb.
    return None

BANDNAME="FeLabs Physik 9 · Realschule NRW"
DATEINAME="FeLabs_Physik_9_Realschule_NRW"
FUSS="FeLabs Physik 9 · Forscherheft"
...
# dafuer, dass 42 gedruckte Codes weiter gelesen werden - bewusst so gelassen.
...
# (dort, wo niemand ein Raetsel setzt). In Klasse 9 sind das AA (Batteriegroesse),
# BGR, CO (aus CO₂), LED, PS und UF - gemessen, nicht aufgezaehlt.
...
# ── Die fast leere Fortsetzungsseite ─────────────────────────────────────────
# Der Umbruch ist gierig: er fuellt jede Seite bis an die Kante und laesst der
# letzten, was uebrig bleibt. In Klasse 5 bis 8 faellt das kaum auf - dort haben
# die Einheiten fuenf bis sieben Bausteine mehr, als auf ein Blatt passen. Klasse
# 9 ist der groesste Band der Reihe, und hier kippte es: Seite 6 trug GENAU EIN
# Themenkaestchen (12 % gefuellt), Seite 267 nur den Notenspiegel (18 %). Genau
# die Sorte Blatt, die in Klasse 8 einmal "nichts als die Fortsetzungszeile und
# einen Strich" trug.
#
# Gegenmittel ohne neue Seite: derselbe Bauplan, dieselbe Seitenzahl, aber ein
# hoeher gelegter Umbruchpunkt. Gesucht wird der Wert, bei dem die LEERSTE Seite
# der Einheit am vollsten wird.
FUELLUNG_MIN = 0.45      # so viel muss die letzte Seite wenigstens tragen
AUSWUCHT_SCHRITT = 12.0  # Einheiten je Probe (5,8 pt)


def _fuellstand(hoehen,seiten):
    """Wo endet die Tinte auf jeder Seite? Aus dem Umbruch abgelesen."""
    return [e[-1][1]+hoehen[e[-1][0]] for e in seiten]


def auswuchten(B,hoehen,seiten):
    """Eine fast leere Fortsetzungsseite gleichmaessig fuellen.

    Ruehrt nur an, was das Kriterium verfehlt, und niemals um den Preis einer
    zusaetzlichen Seite. Gemessen wird an denselben Bausteinhoehen, mit denen
    auch gesetzt wird - die Auswahl kann von der Zeichnung nicht abweichen."""
    if len(seiten)<2: return seiten
    hoch=fd.UNTEN-fd.OBEN
    if (_fuellstand(hoehen,seiten)[-1]-fd.OBEN)/hoch >= FUELLUNG_MIN: return seiten
    beste=seiten; bestwert=min(_fuellstand(hoehen,seiten))
    u=fd.UNTEN
    while u > fd.OBEN+hoch*0.30:
        u-=AUSWUCHT_SCHRITT
        kand=fd.umbrechen(B,hoehen,unten=u)
        if len(kand)!=len(seiten): continue
        w=min(_fuellstand(hoehen,kand))
        if w>bestwert: bestwert,beste=w,kand
    return beste
...
def datenblatt(h,d,y,daten):
    ...
    In Klasse 9 tragen ZWOELF Themen ein Datenblatt (kw1, kw3–kw13): Kapitel 4
    hat fuer diese Fragen keine Simulation, es gibt nichts einzustellen, sondern
    gedruckte Zahlen zu beurteilen. Abschnitt 3 heisst dort deshalb
    "Auswerten & beurteilen", und der QR-Code entfaellt.

    Die Tabelle steht auf der Lesespalte (830 Einheiten = 398 pt) statt wie im
    alten Satz in einer 400 Einheiten schmalen Randspalte. Genau dort riss der
    Text vorher: auf Seite 121 stand gedruckt "liefert Strom unabhaengig vom
    WetterUran muss eingefuehrt werden". fd.tabelle bricht Zellen um."""
    n=len(daten["spalten"])
...
def ch_test(test,chtitel,pn,fuss=None):
    ...
    def b_kopfzeile(h,d,y):
        ...
        yy=y+12; xx=fd.X0+20
        for lab,br in (("Name:",340),("Klasse: 9",0),("Datum:",200)):
...
def divider(num,ch,pn):
    ...
    hoehen=fd.messe_bausteine(B,fd.STIL)
    del boxen[:]
    # Kapitel 1 hat zwoelf Themen, Kapitel 2 fuenfzehn - die Themenliste passt in
    # diesem Band nicht mehr auf ein Blatt. Ausgewuchtet, damit die zweite Seite
    # nicht mit einem einzigen Kaestchen dasteht.
    seiten=auswuchten(B,hoehen,fd.umbrechen(B,hoehen))
    raus=[]
    for si,eintraege in enumerate(seiten):
        im,d=newp(fd.GRUND); h=hp(im,d)
        marke=len(boxen)
...
def book_cover():
    ...
    y+=24
    fd.T(h,fd.X0,y,"Physik 9",fd.schrift("bold",DISPLAY_MITTEL),fd.STIL["h1"])
    y+=LH_DISPLAY+6
...
def footer(h,pn,acc):
    ...
    h.T(ML+34,H-48,"PHYSIK · KLASSE 9 · FORSCHERHEFT",COP(9),GOLD_D)
...
def build_ebook(src, dst, nav, qrpages=None, seitentexte=None,
    ...
    w._root_object[NameObject("/PageMode")]=NameObject("/UseOutlines")
    w.add_metadata({"/Title":"FeLabs PHYSIK 9 – Forscherheft Realschule NRW",
                    "/Author":"Abdullah Lala",
                    "/Subject":"Kräfte · Bewegung · Energie, Arbeit & Leistung · Kraftwerke, Energieversorgung & Klimaschutz",
                    "/Keywords":"FeLabs, Physik, Klasse 9, Realschule NRW, Forscherheft, Kraft, Bewegung, Energie, Klimaschutz, Fassung 1.1, Stand 2026-08-28",
                    "/Creator":"FeLabs"})
