# -*- coding: utf-8 -*-
"""Physik-Gesamtheft Klasse 8 Realschule: vier Kapitel, Test hinter jedem Kapitel.
...
DREI ENTSCHEIDUNGEN DES AUFTRAGGEBERS SIND HIER UMGESETZT
    ...
    3  Der Messwerte-Anhang BLEIBT im Schuelerband. Die Forscherseiten verweisen
       ...
       den Lehrerband wandern, sonst zeigt jede der 25 Seiten ins Leere.
"""
...
# Klasse 8 hat keine Schaltzeichen-Seite und keine eingebauten Schaltbilder.
SCHALTBILD={}
SZ_NACH=None

def _diag(chid,tid):
    # Klasse 8 arbeitet ausschliesslich mit eigenen Einstiegsbildern, nicht mit
    # gezeichneten Szenen. Die Rueckfallebene aus Klasse 5 entfaellt deshalb.
    return None

BANDNAME="FeLabs Physik 8 · Realschule NRW"
DATEINAME="FeLabs_Physik_8_Realschule_NRW"
FUSS="FeLabs Physik 8 · Forscherheft"
...
# (dort, wo niemand ein Raetsel setzt). Fuer Klasse 8 trifft das genau auf LED zu.
...
def datenblatt(h,d,y,daten):
    ...
    Klasse 8 hat keine solche Seite - alle 25 Themen tragen eine Simulation.
...
def ch_test(test,chtitel,pn,fuss=None):
    ...
    def b_kopfzeile(h,d,y):
        ...
        yy=y+12; xx=fd.X0+20
        for lab,br in (("Name:",340),("Klasse: 8",0),("Datum:",200)):
...
def book_cover():
    ...
    y+=24
    fd.T(h,fd.X0,y,"Physik 8",fd.schrift("bold",DISPLAY_MITTEL),fd.STIL["h1"])
    y+=LH_DISPLAY+6
...
def footer(h,pn,acc):
    ...
    h.T(ML+34,H-48,"PHYSIK · KLASSE 8 · FORSCHERHEFT",COP(9),GOLD_D)
...
def build_ebook(src, dst, nav, qrpages=None, seitentexte=None,
    ...
    w._root_object[NameObject("/PageMode")]=NameObject("/UseOutlines")
    w.add_metadata({"/Title":"FeLabs PHYSIK 8 – Forscherheft Realschule NRW",
                    "/Author":"Abdullah Lala",
                    "/Subject":"Spannung & Stromstärke · Widerstand und Ohmsches Gesetz · Elektrische Leistung · Geschwindigkeit & Bewegung",
                    "/Keywords":"FeLabs, Physik, Klasse 8, Realschule NRW, Forscherheft, Elektrizität, Spannung, Widerstand, Geschwindigkeit, Fassung 1.1, Stand 2026-08-28",
                    "/Creator":"FeLabs"})
...
if __name__=="__main__":
    ...
    out=os.path.join(bd,"FeLabs_Physik_8_Realschule_NRW_Druck%s.pdf"%("_weiss" if WEISS else ""))
    pages[0].save(out,"PDF",resolution=150,save_all=True,append_images=pages[1:])
    ...
    if "--druck" in sys.argv:
        import shutil; ziel=os.path.join(ABLAGE, f"FeLabs_Physik_8_Realschule_NRW_Druck.pdf")
        shutil.copy2(out,ziel); print("Druckfassung zusätzlich:",ziel)
    try:
        eb=os.path.join(ABLAGE, f"FeLabs_Physik_8_Realschule_NRW.pdf")
        try:
            ...
            eb=os.path.join(ABLAGE, f"FeLabs_Physik_8_Realschule_NRW_NEU.pdf")
            build_ebook(out, eb, nav, qrpages, seitentexte, mess_start, mess_starts,
                        toc_pn, bk_start)
