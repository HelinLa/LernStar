# -*- coding: utf-8 -*-
"""Bildauftraege Klasse 9 als PDF: eine Seite je fehlendem Einstiegsbild.
...
# Was beim ERSTEN Versuch (02.09.2026) schiefging - eine Zeile je Kennung.
# Geprueft wurde jedes Bild gegen seine Verbotsliste UND gegen den Text der
# Heftseite: Was die Seite erst erarbeiten laesst, darf das Bild nicht zeigen.
BEFUND = {
 "sp1": "Die Kabelenden standen frei aufrecht und trugen Kraft-Strichlein - gezeichnete Kräfte sind ausgeschlossen. Nahaufnahme von oben wie im Motiv.",
 "sp2": "Zwei Bildhälften mit ablesbaren Werten (0,10 A / 0,60 A) - genau die Messung, die das Kind erst machen soll. Ein Aufbau, Display ohne Ziffern.",
 "sp4": "Frontale Gerätefotos voller Aufschriften (COM, V, Ziffern) statt der Szene - die Verbotsliste schließt Zeichen auf den Geräten aus.",
 "wd2": "Die Gerade durch den Nullpunkt war schon eingezeichnet, mit Achsenzahlen und fünftem Punkt - das ist die Entdeckung der Seite. Nur die vier Punkte.",
 "wd3": "Im Bild stand der gerenderte Seitentext (PROBLEM & FRAGE ...) - er stünde doppelt auf der Seite. Nur die drei Drahtrollen zeigen.",
 "wd4": "Ein gezeichneter Schaltplan mit 6 V, R1, R2 - ausgeschlossen sind Schaltplan und lesbare Anzeige. Reale Szene am Steckbrett.",
 "wd5": "Das Messgerät zeigte ablesbar 0,86 A - den Wert, den das Kind erst ermitteln soll. Zifferblatt leer lassen.",
 "wd6": "Widerstandswicklung und Schleifer lagen offen - das Innere ist die Entdeckung der Seite. Geschlossenes Gehäuse, nur der Schieber.",
 "lt1": "Stoppuhr mit ablesbarer Zeit und eine gestrichelte Geister-Batterie mit Ausrufezeichen - gezeichnete Hinweise sind ausgeschlossen.",
 "lt2": "Zwei Stoppuhren (ausdrücklich ausgeschlossen), Netzgerät mit 0,60 A / 12,0 V und beschriftetes Whiteboard - der Merksatz stand damit im Bild.",
 "lt3": "Das Schild nannte 0,34 EUR je kWh - die Seite rechnet mit 30 Cent, das Bild widerspräche dem Heft. Keine ablesbaren Beträge.",
 "lt4": "Zettel mit Rangfolge (1./2./3.), KOSTEN?/NUTZEN? und Pfeil - die Rangfolge soll die Seite erst erarbeiten. Keine lesbaren Texte.",
 "bg3": "Schautafeln mit 50 m in 5 s usw., Häkchenliste und Glühbirnen-Sprechblase - Mias Behauptung war damit bestätigt, bevor gerechnet wurde.",
 "bg4": "Zwei getrennte Bildhälften mit Trennstrich (Rad links, Auto rechts) - Nebeneinander-Bilder sind ausgeschlossen. Eine Szene mit beidem.",
 "bg5": "Dasselbe Auto dreimal mit t = 1 s / 2 s / 3 s - die gleichmäßigen Abstände sind der Merksatz der Seite. Ein Auto, ein Augenblick.",
 "bg6": "Die Kiste siebenmal mit Bewegungsspur den Hang hinab - die wachsenden Abstände sind die Entdeckung der Seite. Ein Augenblick, keine Spur.",
 "bg7": "Drei fertige Linien samt Achsenbeschriftung - die Seite lässt die Linie erst aus Punkten entstehen. Nur Punkte, keine Linie.",
 "bg8": "Die waagerechte Linie war eingezeichnet - exakt der Merksatz der Seite. Leeres Achsenkreuz gehört ohnehin nicht ins Einstiegsbild, sondern eine Szene.",
}

def seitenzahlen():
    """Kennung -> Seite im gedruckten Heft, aus demselben Aufbau wie build_book."""
    pn, raus = 4, {}
...
def _fuss(h, pn, gesamt):
    ...
    h.T(ML + 34, H - 48, "FORSCHERHEFT PHYSIK · KLASSE 8 · BILDAUFTRÄGE", COP(9), GOLD_D)
    if pn:
...
def deckblatt(n):
    ...
    _kopf(h, "WAS ZU TUN IST", f"{ZAHLWORT.get(n, str(n)).capitalize()} Bilder für Klasse 9")
    y = 208
    ...
    kasten = [
        ("Wohin", "In den Ordner „Arbeitsheft-Bilder 8 Klasse“ auf dem Schreibtisch. "
...
def auftragsseite(tid, a, seite, nr, gesamt):
    ...
    if tid in BEFUND:
        bz = h.wrap("Erster Versuch, zurückgestellt: " + BEFUND[tid], AV(14.5), W - 2 * ML - 78)
        bh2 = 26 + len(bz) * 25
        h.R(ML, y, W - ML, y + bh2, 10, fill=(250, 231, 228), outline=(186, 84, 70), w=1.4)
        h.circ(ML + 30, y + bh2 / 2, 13, fill=(186, 84, 70))
        h.T(ML + 30, y + bh2 / 2 + 1, "!", AVB(16), CREAM, anchor="mm")
        h.para(ML + 56, y + 13, "Erster Versuch, zurückgestellt: " + BEFUND[tid],
               AV(14.5), (128, 52, 42), W - 2 * ML - 78, 25)
        y += bh2 + 26
...
if __name__ == "__main__":
    ...
    w.add_metadata({"/Title": "Bildaufträge · Forscherheft Physik Klasse 8",
                    "/Author": "Abdullah Lala", "/Subject": "Fehlende Einstiegsbilder"})
    ziel = os.path.expanduser("~/Desktop/Bildauftraege_Klasse8.pdf")
